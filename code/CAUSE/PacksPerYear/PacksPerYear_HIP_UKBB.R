...
#We are going to run CAUSE for PacksPerYear smoking as exposure and HIP as outcome
...
HIP <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/HIP/HIP_UKBB_Combined_Curated_FULL.txt")
...
summary(HIP$eaf.exposure) #perfect.
...
HIP <- HIP[which(HIP$effect_allele.exposure%in%yes_vect),] #all of them, as expected.
HIP <- HIP[which(HIP$other_allele.exposure%in%yes_vect),] #all of them, as expected.

################################################################
#Let's first check whether we have chr_pos for all the variants#
################################################################

HIP$chr_pos <- paste(HIP$chr.exposure, ":", HIP$pos.exposure, sep = "")

HIP <- HIP[order(HIP$chr_pos),]

head(HIP) #seems all is OK
tail(HIP) #seems all is OK
...
#For once we have the data formatted in the same way and they come from the same panel, 
#hence we do not need the chr_pos:

X <- gwas_merge(PacksPerYear_, HIP, snp_name_cols = c("SNP", "SNP"), 
                beta_hat_cols = c("beta.exposure", "beta.exposure"), 
                se_cols = c("se.exposure", "se.exposure"), 
                A1_cols = c("effect_allele.exposure", "effect_allele.exposure"), 
                A2_cols = c("other_allele.exposure", "other_allele.exposure"))
...
saveRDS(params, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/PacksPerYear/dataframes/params_PacksPerYear_HIP_UKBB.rds")
saveRDS(X, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/PacksPerYear/dataframes/X_PacksPerYear_HIP_UKBB.rds")

X <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/PacksPerYear/dataframes/X_PacksPerYear_HIP_UKBB.rds")
params <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/PacksPerYear/dataframes/params_PacksPerYear_HIP_UKBB.rds")
...
write.table(PacksPerYear_pruned, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/PacksPerYear/dataframes/PacksPerYear_HIP_UKBB_PRUNED.txt", col.names = TRUE, row.names = FALSE, quote = FALSE)

#################
#Let's run CAUSE#
#################

top_PacksPerYear_pruned_df <- read.table("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/PacksPerYear/dataframes/PacksPerYear_HIP_UKBB_PRUNED.txt", header = TRUE, stringsAsFactors = FALSE)

top_PacksPerYear_pruned_snps <- top_PacksPerYear_pruned_df$SNP

#Full version:

res <- cause(X=X, variants = top_PacksPerYear_pruned_snps, param_ests = params, qalpha = 1, qbeta = 2)

saveRDS(res, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/PacksPerYear/dataframes/RES_PacksPerYear_to_HIP_UKBB_FULL.rds")

res <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/PacksPerYear/dataframes/RES_PacksPerYear_to_HIP_UKBB_FULL.rds")

res$elpd

#model1  model2 delta_elpd se_delta_elpd          z
#1    null sharing -20.412864      5.631643 -3.6246732
#2    null  causal -18.950045      6.569457 -2.8845680
#3 sharing  causal   1.462819      1.805766  0.8100822

#Sounds really promising:

summary(res)

#p-value testing that causal model is a better fit:  0.79 
#Posterior medians and  95 % credible intervals:
#  model     gamma                eta                   q                  
#[1,] "Sharing" NA                   "0.19 (0.13, 0.27)"   "0.73 (0.49, 0.92)"
#[2,] "Causal"  "0.15 (-0.03, 0.25)" "-0.13 (-0.29, 0.26)" "0.25 (0.02, 0.86)"
