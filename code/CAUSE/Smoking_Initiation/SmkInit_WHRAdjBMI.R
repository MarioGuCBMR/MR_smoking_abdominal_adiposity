...
swapping_snps_2_in_lifetime <- swapping_snps_2[which(swapping_snps_2$new_rs%in%SMK_$RSID),] #129 do not need to be changed. Why? Because GIANT has already those. And lifetime too.

#HENCE, lifetime old are those that need to be changed!!

swapping_snps_2_in_SMK_old <- swapping_snps_2[which(swapping_snps_2$old_rs%in%SMK_$RSID),] #0
...
X_UKBB <- gwas_merge(SMK_, WHRAdjBMI_, snp_name_cols = c("chr_pos", "chr_pos"), 
...
X_GIANT_in_UKBB <- X_GIANT[which(X_GIANT$snp%in%X_UKBB$snp),] #255. SAME.
...
which(duplicated(X_end$snp) == TRUE) #0

set.seed(100)
varlist <- with(X_end, sample(snp, size=1000000, replace=FALSE))
params <- est_cause_params(X_end, varlist)

saveRDS(params, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/params_SMK_WHRAdjBMI_JULY.rds")
saveRDS(X_end, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/X_SMK_WHRAdjBMI_JULY.rds")

params <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/params_SMK_WHRAdjBMI_JULY.rds")
X_end_2 <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/X_SMK_WHRAdjBMI_JULY.rds")
...
write.table(SMK_pruned, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/SMK_WHRAdjBMI_PRUNED_JULY.txt", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
just_check <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/SMK_Smoking/dataframes/SMK_WHRAdjBMI_PRUNED.txt") #exactly the same
...
top_SMK_pruned_df <- read.table("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/SMK_WHRAdjBMI_PRUNED_JULY.txt", header = TRUE, stringsAsFactors = FALSE)
...
saveRDS(res, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/RES_SMK_to_WHRAdjBMI_FULL_JULY.rds")

res <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/RES_SMK_to_WHRAdjBMI_FULL_JULY.rds")

res$elpd

#model1  model2  delta_elpd se_delta_elpd         z
#1    null sharing -53.8413674     9.5815844 -5.619255
#2    null  causal -54.8261197     9.8723051 -5.553528
#3 sharing  causal  -0.9847522     0.3844367 -2.561546

#Sounds really promising:

summary(res)

#p-value testing that causal model is a better fit:  0.0052 
#Posterior medians and  95 % credible intervals:
#  model     gamma              eta                q                  
#[1,] "Sharing" NA                 "0.08 (0.06, 0.1)" "0.84 (0.61, 0.97)"
#[2,] "Causal"  "0.07 (0.03, 0.1)" "0 (-0.16, 0.16)"  "0.18 (0, 0.86)" 
