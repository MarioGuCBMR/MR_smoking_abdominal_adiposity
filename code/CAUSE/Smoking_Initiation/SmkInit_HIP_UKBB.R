...
#We are going to run CAUSE for SmkInit as exposure and HIP as outcome
...
HIP$chr_pos <- paste("chr", HIP$chr.exposure, ":", HIP$pos.exposure, sep = "")
...
X <- gwas_merge(SMK_, HIP, snp_name_cols = c("chr_pos", "chr_pos"), 
...
HIP[which(HIP$chr_pos == "chr10:100000625"),] #PERFECT. Also checked in PS.
...
HIP_not_clean <- HIP[which(HIP$chr_pos%in%X_not_clean$snp),]

HIP_not_clean <- HIP_not_clean[order(match(HIP_not_clean$chr_pos, X_not_clean$snp)),]

#Let's do some checks:

which(HIP_not_clean$chr_pos != X_not_clean$snp) #cool.

head(HIP_not_clean$chr_pos)
head(X_not_clean$snp) 

#Perfect, now we retrieve the RSIDs:

X_not_clean$snp <- HIP_not_clean$SNP #some SNPs aare there. The other ones are probably unretrievable.

#Finally, let's check in PhenoScanner if these exist
#and get the data properly

check_ps_1 <- phenoscanner::phenoscanner(HIP_not_clean$chr_pos[seq(1,100)])
check_ps_2 <- phenoscanner::phenoscanner(HIP_not_clean$chr_pos[seq(101, 126)])
...
saveRDS(params, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/params_SMKInit_HIP_UKBB.rds")
saveRDS(X_end, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/X_SMKInit_HIP_UKBB.rds")
...
write.table(SMK_pruned, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/SMKInit_HIP_UKBB_PRUNED.txt", col.names = TRUE, row.names = FALSE, quote = FALSE)

#################
#Let's run CAUSE#
#################

top_smk_pruned_df <- read.table("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/SMKInit_HIP_UKBB_PRUNED.txt", header = TRUE, stringsAsFactors = FALSE)
...
saveRDS(res, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/RES_SMKinit_to_HIP_UKBB_FULL.rds")

res <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Smoking_Initiation/dataframes/RES_SMKinit_to_HIP_UKBB_FULL.rds")

res$elpd

#model1  model2 delta_elpd se_delta_elpd         z
#1    null sharing -39.334027     7.1879612 -5.472209
#2    null  causal -40.681591     7.6274413 -5.333583
#3 sharing  causal  -1.347564     0.4442561 -3.033304

#Sounds really promising:

summary(res)

#p-value testing that causal model is a better fit:  0.0012 
#Posterior medians and  95 % credible intervals:
#  model     gamma               eta                q                 
#[1,] "Sharing" NA                  "0.1 (0.08, 0.13)" "0.89 (0.7, 0.98)"
#[2,] "Causal"  "0.09 (0.05, 0.13)" "0 (-0.18, 0.19)"  "0.18 (0, 0.86)" 
