...
#We are going to run CAUSE for LifetimeInit as exposure and HIP as outcome
...
summary(HIP$FreqAllele1HapMapCEU) #perfect.
...
HIP <- HIP[which(HIP$Allele1%in%yes_vect),] #all of them, as expected.
HIP <- HIP[which(HIP$Allele2%in%yes_vect),] #all of them, as expected.

###############################################################################
#Let's do first a match of those SNPs that do not have chromosome and position#
###############################################################################

HIP_missing <- HIP[which(HIP$chr_pos_37 == "-"),]

which(HIP_missing$MarkerName%in%Lifetime_$RSID) #none of them. 
...
index_merged_1 <- which(merged_rs$old_rs%in%HIP_missing$MarkerName) #Only 2! 
index_merged_2 <- which(merged_rs$new_rs%in%HIP_missing$MarkerName) #Only 2! 
...
X <- gwas_merge(Lifetime_, HIP, snp_name_cols = c("chr_pos", "chr_pos_37"), 
...
HIP[which(HIP$chr_pos_37 == "chr1:752566"),] #PERFECT. Also checked in PS.
...
HIP_not_clean <- HIP[which(HIP$chr_pos_37%in%X_not_clean$snp),]

HIP_not_clean <- HIP_not_clean[order(match(HIP_not_clean$chr_pos_37, X_not_clean$snp)),]

#Let's do some checks:

which(HIP_not_clean$chr_pos_37 != X_not_clean$snp) #cool.

head(HIP_not_clean$chr_pos_37)
head(X_not_clean$snp) 

#Perfect, now we retrieve the RSIDs:

X_not_clean$snp <- HIP_not_clean$MarkerName
...
check_ps <- phenoscanner::phenoscanner(HIP_not_clean$chr_pos_37)
...
saveRDS(params, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/params_Lifetime_HIP.rds")
saveRDS(X_end, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/X_Lifetime_HIP.rds")
...
write.table(Lifetime_pruned, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/Lifetime_HIP_PRUNED.txt", col.names = TRUE, row.names = FALSE, quote = FALSE)

#################
#Let's run CAUSE#
#################

top_Lifetime_pruned_df <- read.table("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/Lifetime_HIP_PRUNED.txt", header = TRUE, stringsAsFactors = FALSE)
...
saveRDS(res, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/RES_Lifetime_to_HIP_FULL.rds")

res <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/RES_Lifetime_to_HIP_FULL.rds")

res$elpd

#model1  model2 delta_elpd se_delta_elpd         z
#1    null sharing -8.9205133     3.8917405 -2.292165
#2    null  causal -9.6624772     4.4106462 -2.190717
#3 sharing  causal -0.7419639     0.5321962 -1.394155

#Sounds really promising:

summary(res)

#p-value testing that causal model is a better fit:  0.082 
#Posterior medians and  95 % credible intervals:
#  model     gamma                eta                  q                  
#[1,] "Sharing" NA                   "0.2 (0.1, 0.48)"    "0.56 (0.19, 0.91)"
#[2,] "Causal"  "0.13 (-0.01, 0.28)" "0.01 (-0.54, 0.72)" "0.19 (0, 0.86)"  
