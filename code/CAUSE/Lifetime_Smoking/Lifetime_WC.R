...
#We are going to run CAUSE for LifetimeInit as exposure and WC as outcome
...
summary(WC$FreqAllele1HapMapCEU) #perfect.
...
WC <- WC[which(WC$Allele1%in%yes_vect),] #all of them, as expected.
WC <- WC[which(WC$Allele2%in%yes_vect),] #all of them, as expected.

###############################################################################
#Let's do first a match of those SNPs that do not have chromosome and position#
###############################################################################

WC_missing <- WC[which(WC$chr_pos_37 == "-"),]

which(WC_missing$MarkerName%in%Lifetime_$RSID) #none of them. 
...
index_merged_1 <- which(merged_rs$old_rs%in%WC_missing$MarkerName) #Only 2! 
index_merged_2 <- which(merged_rs$new_rs%in%WC_missing$MarkerName) #Only 2! 
...
X <- gwas_merge(Lifetime_, WC, snp_name_cols = c("chr_pos", "chr_pos_37"), 
...
WC[which(WC$chr_pos_37 == "chr1:752566"),] #PERFECT. Also checked in PS.
...
WC_not_clean <- WC[which(WC$chr_pos_37%in%X_not_clean$snp),]

WC_not_clean <- WC_not_clean[order(match(WC_not_clean$chr_pos_37, X_not_clean$snp)),]

#Let's do some checks:

which(WC_not_clean$chr_pos_37 != X_not_clean$snp) #cool.

head(WC_not_clean$chr_pos_37)
head(X_not_clean$snp) 

#Perfect, now we retrieve the RSIDs:

X_not_clean$snp <- WC_not_clean$MarkerName
...
check_ps <- phenoscanner::phenoscanner(WC_not_clean$chr_pos_37)
...
saveRDS(params, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/params_LifetimeInit_WC.rds")
saveRDS(X_end, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/X_LifetimeInit_WC.rds")
...
write.table(Lifetime_pruned, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/LifetimeInit_WC_PRUNED.txt", col.names = TRUE, row.names = FALSE, quote = FALSE)

#################
#Let's run CAUSE#
#################

top_Lifetime_pruned_df <- read.table("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/LifetimeInit_WC_PRUNED.txt", header = TRUE, stringsAsFactors = FALSE)
...
saveRDS(res, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/RES_Lifetime_to_WC_FULL.rds")

res <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/Lifetime_Smoking/dataframes/RES_Lifetime_to_WC_FULL.rds")

res$elpd

#model1  model2 delta_elpd se_delta_elpd         z
#1    null sharing  -31.15055     7.0138222 -4.441309
#2    null  causal  -32.49723     7.4593945 -4.356551
#3 sharing  causal   -1.34668     0.4637913 -2.903634

#Sounds really promising:

summary(res)

#p-value testing that causal model is a better fit:  0.0018 
#Posterior medians and  95 % credible intervals:
#  model     gamma              eta                 q                  
#[1,] "Sharing" NA                 "0.26 (0.19, 0.38)" "0.81 (0.53, 0.97)"
#[2,] "Causal"  "0.22 (0.1, 0.35)" "0 (-0.49, 0.58)"   "0.19 (0, 0.86)"  
