...
WHRAdjBMI <- WHRAdjBMI[order(WHRAdjBMI$chr_pos_37),]

head(WHRAdjBMI)
tail(WHRAdjBMI)

#We have some SNPs that do not present chromosome and positions and
#hence, we cannot work with them.
#Even with liftover we had issues.

#######################
#Let's make some tests#
#######################

WHRAdjBMI_not_liftover_weird <- WHRAdjBMI[which(WHRAdjBMI$chr_pos_37 == "-"),]

#Let's check whether the RSIDs can be matched with Smoking data:

which(WHRAdjBMI_not_liftover_weird$rs_id%in%CigDay_$RSID) #none of them. 

#The issue might be because they aren't in the chromsome and position form.
#But not even with liftover we could save these SNPs...
#Hence, let's try to match them with RSIDs, but considering that some of them
#might be merged.
...
index_merged_old <- which(merged_rs$old_rs%in%WHRAdjBMI_not_liftover_weird$rs_id) #Only 2! 
index_merged_new <- which(merged_rs$new_rs%in%WHRAdjBMI_not_liftover_weird$rs_id) #We have 553. That means that we have A LOT of duplicates. All of them were too new it seems!! 
...
WHRAdjBMI <- WHRAdjBMI[which(WHRAdjBMI$Effect_allele%in%yes_vect),] #all of them, as expected.
WHRAdjBMI <- WHRAdjBMI[which(WHRAdjBMI$Other_allele%in%yes_vect),] #all of them, as expected.
...
X <- gwas_merge(CigDay_, WHRAdjBMI, snp_name_cols = c("chr_pos", "chr_pos_37"), 
...
WHRAdjBMI[which(WHRAdjBMI$chr_pos_37 == "chr10:100004906"),] #PERFECT.
...
saveRDS(params, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/CigDay/params_CigDay_WHRAdjBMIsmk.rds")
saveRDS(X_end, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/CigDay/X_CigDay_WHRAdjBMIsmk.rds")

#checking the old one:

check <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/CigDay/CigDay-WHRAdjBMIsmk/dataframes/old/X_CigDayInit_WHR.rds") #SAME SNPs
params_old <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/CigDay/CigDay-WHRAdjBMIsmk/dataframes/old/params_CigDayInit_WHRAdjBMIsmk.rds")

params_old$rho #same
params$rho #same

params$prior #same
params_old$prior #same

#The results will be identical, but let's try just in case...
...
write.table(CigDay_pruned, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/CigDay/CigDay-WHRAdjBMIsmk/dataframes/CigDay_WHRAdjBMIsmk_PRUNED.txt", col.names = TRUE, row.names = FALSE, quote = FALSE)

check <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/CigDay/CigDay-WHRAdjBMIsmk/dataframes/old/CigDayInit_WHRAdjBMIsmk_PRUNED.txt") #same SNPs as before

#################
#Let's run CAUSE#
#################

top_CigDay_pruned_df <- read.table("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/CigDay/CigDay-WHRAdjBMIsmk/dataframes/CigDay_WHRAdjBMIsmk_PRUNED.txt", header = TRUE, stringsAsFactors = FALSE)
...
saveRDS(res, file = "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/CigDay/CigDay-WHRAdjBMIsmk/dataframes/RES_CigDay_to_WHRAdjBMIsmk_FULL.rds")

res <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/CAUSE/CigDay/CigDay-WHRAdjBMIsmk/dataframes/RES_CigDay_to_WHRAdjBMIsmk_FULL.rds")

res$elpd

#model1  model2 delta_elpd se_delta_elpd         z
#1    null sharing  0.4659544     0.4090162 1.1392075
#2    null  causal  0.7882460     1.0863001 0.7256246
#3 sharing  causal  0.3222917     0.6853691 0.4702454

#Sounds really promising:

summary(res)

#p-value testing that causal model is a better fit:  0.68 
#Posterior medians and  95 % credible intervals:
#  model     gamma              eta                  q               
#[1,] "Sharing" NA                 "0.04 (-0.63, 0.46)" "0.12 (0, 0.74)"
#[2,] "Causal"  "0.02 (-0.1, 0.1)" "-0.04 (-0.7, 0.36)" "0.14 (0, 0.86)"
