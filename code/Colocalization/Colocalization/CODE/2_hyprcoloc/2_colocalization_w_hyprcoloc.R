...
coloc_df <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/Colocalization/Colocalization/info_4_loci/1_ss_of_all_traits/15_78396129_79396129.txt")

########################################################
#STEP 1: let's define the loci that we are going to use#
########################################################

trait.names <- c("cigday", "whradjbmi", "shbg", "cortisol")

betas <- coloc_df %>%
  select(BETA.cigday, BETA.whradjbmi, BETA.shbg, BETA.cortisol)

print(dim(betas))

betas <- as.matrix(betas)
rownames(betas) <- coloc_df$variant
colnames(betas) <-  c("cigday", "whradjbmi", "shbg", "cortisol")

ses <- coloc_df %>%
  select(SE.cigday, SE.whradjbmi, SE.shbg, SE.cortisol)

print(dim(ses))

ses <- as.matrix(ses)
rownames(ses) <- coloc_df$variant
colnames(ses) <-c("cigday", "whradjbmi", "shbg", "cortisol")
...
#1         1   None             NA        0.0058            NA                         NA        cigday
#2         2   None             NA        0.0001            NA                         NA      cortisol
#3         3   None             NA        0.0000            NA                         NA     whradjbmi
