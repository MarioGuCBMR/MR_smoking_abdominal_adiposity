...
whr <- fread("C:/Users/zlc436/Desktop/SMK_WHRAdjBMI/GIANT_2015_WHR_COMBINED_EUR.txt") 
...
BMI_match <- BMI[which(BMI$SNP%in%whr_maf$MarkerName),] #most of them:

whr_Pulit <- whr_maf[which(whr_maf$MarkerName%in%BMI_match$SNP),]
...
BMI_match <- BMI_match[order(match(BMI_match$SNP, whr_Pulit$MarkerName)),]

#Let's check if we did it properly:

which(BMI_match$SNP != whr_Pulit$MarkerName) #perfect.
...
whr_missing <- whr_maf[which(!(whr_maf$MarkerName%in%BMI_match$SNP)),]
...
write.table(for_nexus, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WHR/whr_combined_4_Nexus.txt", sep = " ", quote = FALSE, col.names = FALSE, row.names = FALSE)

####################
#Let's recover them#
####################

recovered_snps <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WHR/whr_combined_in_Nexus.txt")
...
write.table(chr_pos_18, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WHR/WHR_4_liftover.txt", quote = FALSE, row.names = FALSE, col.names =  FALSE)

#########################################################################################
#Now we are gonna redo Liftover without the failing conversions so we can match the data#
#########################################################################################

failed_conversions <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WHR/Failed_Conversions.txt")
...
write.table(whr_missing_2_new$chr_pos_18, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WHR/WHR_4_liftover_curated.txt", quote = FALSE, row.names = FALSE, col.names =  FALSE)

converted_data <- as.data.frame(read.table("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WHR/whr_liftover_correct.bed",header = FALSE, sep="\t",stringsAsFactors=FALSE, quote=""))
...
fwrite(whr_end, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WHR/WHR_combined_Curated.txt")

############################################
#Finally, let's see if we can load the data#
############################################

check <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WHR/WHR_combined_Curated.txt")
