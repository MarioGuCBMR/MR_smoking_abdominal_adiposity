...
#This is a quick code to check how much do we need to clean the hipadjbmi data.
...
hipadjbmi <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/RAW_Data/GIANT_2015_HIPadjBMI_COMBINED_EUR.txt") 

#Let's check what columns do we have:

hipadjbmi <- hipadjbmi[order(hipadjbmi$MarkerName),]
...
#The hipadjbmi data has some of the SNPs without allele frequency.
#We cannot know if the imputation was done correctly...
#Hence, we are gonna remove them.

hipadjbmi_maf <- hipadjbmi[which(hipadjbmi$FreqAllele1HapMapCEU > 0.01),]
hipadjbmi_maf <- hipadjbmi_maf[which(hipadjbmi_maf$FreqAllele1HapMapCEU < 0.99),]

summary(hipadjbmi_maf$FreqAllele1HapMapCEU) #perfect.

#Now the INFO, do we have that data?

colnames(hipadjbmi_maf) #we do not. 
...
BMI_match <- BMI[which(BMI$SNP%in%hipadjbmi_maf$MarkerName),] #most of them:

hipadjbmi_Pulit <- hipadjbmi_maf[which(hipadjbmi_maf$MarkerName%in%BMI_match$SNP),]
...
BMI_match <- BMI_match[order(match(BMI_match$SNP, hipadjbmi_Pulit$MarkerName)),]

#Let's check if we did it properly:

which(BMI_match$SNP != hipadjbmi_Pulit$MarkerName) #perfect.

head(BMI_match$SNP)
head(hipadjbmi_Pulit$MarkerName)

#We can match the chr_pos:

hipadjbmi_Pulit$chr_pos_37 <- BMI_match$chr_pos
...
hipadjbmi_missing <- hipadjbmi_maf[which(!(hipadjbmi_maf$MarkerName%in%BMI_match$SNP)),]

#Let's see how many of them can be recovered from SNP Nexus:

for_nexus <- hipadjbmi_missing
...
#They are the same as we had in hipadjbmiAdjBMI combined.
...
hipadjbmi_missing <- hipadjbmi_missing[order(hipadjbmi_missing$MarkerName),]

hipadjbmi_missing_gotcha <- hipadjbmi_missing[seq(1,3),]

#After checking, the SNP match. 
#Hence:

hipadjbmi_missing_gotcha$MarkerName <- BMI_gotcha$SNP

hipadjbmi_missing_gotcha$chr_pos_37 <- BMI_gotcha$chr_pos
...
write.table(for_nexus, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/HIPAdjBMI_comb/hipadjbmi_combined_4_Nexus.txt", sep = " ", quote = FALSE, col.names = FALSE, row.names = FALSE)

####################
#Let's recover them#
####################

recovered_snps <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/HIPAdjBMI_comb/hipadjbmi_combined_in_Nexus.txt")
...
hipadjbmi_recovered <- hipadjbmi_missing[which(hipadjbmi_missing$MarkerName%in%recovered_snps$dbSNP),]

recovered_snps <- recovered_snps[order(match(recovered_snps$dbSNP, hipadjbmi_recovered$MarkerName)),]

#Doing some checkity checks:

which(recovered_snps$dbSNP != hipadjbmi_recovered$MarkerName)

head(recovered_snps$dbSNP)
head(hipadjbmi_recovered$MarkerName)
...
hipadjbmi_recovered$chr_pos_37 <- recovered_snps$chr_pos

###############################
#Let's check the missing ones:#
###############################

hipadjbmi_missing_2 <- hipadjbmi_missing[which(!(hipadjbmi_missing$MarkerName%in%recovered_snps$dbSNP)),]

View(hipadjbmi_missing_2)

#The first three were already recovered, so let's remove them:

hipadjbmi_missing_2 <- hipadjbmi_missing_2[-seq(1,3),]

head(hipadjbmi_missing_2)
...
whr_liftovered <- whr_curated[which(whr_curated$MarkerName%in%hipadjbmi_missing_2$MarkerName),] #2463/2469. SO CLOSE!
...
hipadjbmi_missing_2_matched <- hipadjbmi_missing_2[which(hipadjbmi_missing_2$MarkerName%in%whr_liftovered$MarkerName),]

#Now we reorder them and get the chr and position:

hipadjbmi_missing_2_matched <- hipadjbmi_missing_2_matched[order(match(hipadjbmi_missing_2_matched$MarkerName, whr_liftovered$MarkerName)),]

which(hipadjbmi_missing_2_matched$MarkerName != whr_liftovered$MarkerName) #perfect!!

head(hipadjbmi_missing_2_matched$MarkerName) #perfect
head(whr_liftovered$MarkerName) #perfect

hipadjbmi_missing_2_matched$chr_pos_37 <- whr_liftovered$chr_pos_37
...
hipadjbmi_missing_3 <- hipadjbmi_missing_2[which(!(hipadjbmi_missing_2$MarkerName%in%hipadjbmi_missing_2_matched$MarkerName)),]

#181 SNPs! Not bad

results_1 <- phenoscanner::phenoscanner(hipadjbmi_missing_3$MarkerName[seq(1,100)])
results_2 <- phenoscanner::phenoscanner(hipadjbmi_missing_3$MarkerName[seq(101,181)])
...
hipadjbmi_missing_3_matched <- hipadjbmi_missing_3[which(hipadjbmi_missing_3$MarkerName%in%results$snp),]

#Let's try to match them:

hipadjbmi_missing_3_matched <- hipadjbmi_missing_3_matched[order(match(hipadjbmi_missing_3_matched$MarkerName, results$snp)),]

which(hipadjbmi_missing_3_matched$MarkerName != results$snp)

head(hipadjbmi_missing_3_matched$MarkerName)
head(results$snp)

hipadjbmi_missing_3_matched$chr_pos_37 <- results$hg19_coordinates

###################################################
#We will do our best in the CAUSE pipeline then...#
###################################################

hipadjbmi_missing_4 <- hipadjbmi_missing_3[which(!(hipadjbmi_missing_3$MarkerName%in%results$snp)),]

hipadjbmi_missing_4$chr_pos_37 <- "-" #to include all variants just in case.
...
#The hipadjbmi_missing_3_matched did not work for hipadjbmi.

hipadjbmi_end <- rbind(hipadjbmi_Pulit, hipadjbmi_recovered, hipadjbmi_missing_gotcha, hipadjbmi_missing_2_matched, hipadjbmi_missing_3_matched, hipadjbmi_missing_4) #PERFECT number. It matches the data perfectily.

dim(hipadjbmi_end)
dim(hipadjbmi_maf)

fwrite(hipadjbmi_end, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/HIPAdjBMI_comb/hipadjbmi_combined_Curated.txt")

############################################
#Finally, let's see if we can load the data#
############################################

check <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/hipadjbmi/hipadjbmi_combined_Curated.txt")
