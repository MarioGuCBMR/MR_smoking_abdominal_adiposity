...
wcadjbmi <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/RAW_Data/GIANT_2015_WCadjBMI_COMBINED_EUR.txt") 
...
BMI_match <- BMI[which(BMI$SNP%in%wcadjbmi_maf$MarkerName),] #most of them:

wcadjbmi_Pulit <- wcadjbmi_maf[which(wcadjbmi_maf$MarkerName%in%BMI_match$SNP),]
...
BMI_match <- BMI_match[order(match(BMI_match$SNP, wcadjbmi_Pulit$MarkerName)),]

#Let's check if we did it properly:

which(BMI_match$SNP != wcadjbmi_Pulit$MarkerName) #perfect.
...
wcadjbmi_missing <- wcadjbmi_maf[which(!(wcadjbmi_maf$MarkerName%in%BMI_match$SNP)),]
...
check_3_snps <- as.data.frame(read.table("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/wcadjbmi/check_3_SNPs_in_liftover.bed",header = FALSE, sep="\t",stringsAsFactors=FALSE, quote=""))
...
write.table(for_nexus, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WCAdjBMI/WCAdjBMI_combined_4_Nexus.txt", sep = " ", quote = FALSE, col.names = FALSE, row.names = FALSE)

####################
#Let's recover them#
####################

recovered_snps <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WCAdjBMI/WCAdjBMI_combined_in_Nexus.txt")
...
whr_liftovered <- whr_curated[which(whr_curated$MarkerName%in%wcadjbmi_missing_2$MarkerName),] #2463/2469. SO CLOSE!
...
results_2 <- phenoscanner::phenoscanner(wcadjbmi_missing_3$MarkerName[seq(101,182)])
...
fwrite(wcadjbmi_end, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WCAdjBMI/WCAdjBMI_combined_Curated.txt")

############################################
#Finally, let's see if we can load the data#
############################################

check <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/Curating_data/Curated_data/WCAdjBMI/WCAdjBMI_combined_Curated.txt")
