...
#This is the 2SMR code to run packyears_smk > wc.
...
#We get 13/14! One is in the MHC.
...
wc_match_by_rsid <- wc[which(wc$MarkerName%in%packyears_smk_gw_ind$SNP),] #7/13
...
wc_match_by_chr_pos <- wc[which(wc$chr_pos_37%in%packyears_smk_gw_ind$chr_pos_37),] #7/13
...
packyears_smk_gw_ind_missing <- packyears_smk_gw_ind[which(!(packyears_smk_gw_ind$chr_pos_37%in%wc_match_by_chr_pos$chr_pos_37)),]
...
wc_proxies <- wc[which(wc$chr_pos_37%in%proxies_df_clean_curated$Coord),] 

#Now let's match the data:

packyears_smk_proxies_match <- packyears_smk_proxies[which(packyears_smk_proxies$chr_pos_37%in%wc_proxies$chr_pos_37),] #38
wc_proxies_match <- wc_proxies[which(wc_proxies$chr_pos_37%in%packyears_smk_proxies$chr_pos_37),] #38

#Let's check with chromsome and position:

packyears_smk_proxies_match_2 <- packyears_smk_proxies[which(packyears_smk_proxies$chr_pos_37%in%wc_proxies$chr_pos_37),] #38
...
packyears_smk_gw_ind_not_missing <- packyears_smk_gw_ind[which(packyears_smk_gw_ind$chr_pos_37%in%wc_match_by_chr_pos$chr_pos_37),]

packyears_smk_gw_ind_plus_proxies <- rbind(packyears_smk_gw_ind_not_missing, packyears_smk_proxies_match) #we get 52 + 699 = 751 SNPs. Makes total sense.
...
dat_1_pre <- harmonise_data(packyears_smk_gw_ind_post_proxies, wc_outcome, action = 3) #104/104: perfect.
...
#1 Pack years adult smoking         WC      WC Pack years adult smoking                  MR Egger   10 -0.212989305 0.13965996 0.1657545
#2 Pack years adult smoking         WC      WC Pack years adult smoking           Weighted median   10 -0.063283016 0.04496637 0.1593263
#3 Pack years adult smoking         WC      WC Pack years adult smoking Inverse variance weighted   10  0.004135939 0.07828981 0.9578685
#4 Pack years adult smoking         WC      WC Pack years adult smoking               Simple mode   10  0.085902706 0.14327796 0.5635913
#5 Pack years adult smoking         WC      WC Pack years adult smoking             Weighted mode   10 -0.067943725 0.04378892 0.1551682
...
#  tau^2 = 0.0703 [0.0164; 0.4196]; tau = 0.2652 [0.1281; 0.6478]
#I^2 = 69.4% [41.1%; 84.1%]; H = 1.81 [1.30; 2.51]
...
#1  Egger fixed effects 0.009739227 0.002967145  0.0039237286 0.01555472 0.001029436
#2 Egger random effects 0.009739227 0.005418588 -0.0008810104 0.02035946 0.036138166

#[[1]]$Q
#Method        Q df            P
#1   Q_ivw 37.45378  9 2.183646e-05
#2 Q_egger 26.67992  8 8.027888e-04
#3  Q_diff 10.77386  1 1.029436e-03
...
#4 Rucker   10 -0.2129893 0.13966 -0.4867178 0.06073919 0.1657545
...
tiff("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/2SMR/PackYears/PackYears_WC/packyears_smk_wc_original_20082021.tiff", units="in", width=10, height=10, res=300)
mr_plots(dat_1_filt)
dev.off()
...
#rs1364063: Arm fat percentage left
...
#id.exposure id.outcome outcome                 exposure                    method nsnp           b         se       pval
#1 Pack years adult smoking         WC      WC Pack years adult smoking                  MR Egger    8 -0.21722295 0.10056999 0.07408524
#2 Pack years adult smoking         WC      WC Pack years adult smoking           Weighted median    8 -0.06219272 0.04415623 0.15899197
#3 Pack years adult smoking         WC      WC Pack years adult smoking Inverse variance weighted    8 -0.01233367 0.06064976 0.83885455
#4 Pack years adult smoking         WC      WC Pack years adult smoking               Simple mode    8  0.01163873 0.16059415 0.94425285
#5 Pack years adult smoking         WC      WC Pack years adult smoking             Weighted mode    8 -0.07945443 0.04677687 0.13320287
...
#Quantifying heterogeneity:
#  tau^2 = 0.0255 [0.0000; 0.1834]; tau = 0.1597 [0.0000; 0.4283]
#I^2 = 53.9% [0.0%; 79.2%]; H = 1.47 [1.00; 2.19]
...
#[[1]]$intercept
#Method   Estimate          SE      CI_low     CI_upp           P
#1  Egger fixed effects 0.01000819 0.003593345 0.002965361 0.01705101 0.005349458
#2 Egger random effects 0.01000819 0.004326687 0.001528037 0.01848834 0.010357760

#[[1]]$Q
#Method         Q df           P
#1   Q_ivw 16.456245  7 0.021259711
#2 Q_egger  8.698901  6 0.191233141
#3  Q_diff  7.757344  1 0.005349458
...
#[[1]]$selected
#Method nsnp   Estimate         SE     CI_low     CI_upp          P
#3 Rucker    8 -0.2172229 0.08352411 -0.3809272 -0.0535187 0.04062292
...
tiff("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/2SMR/PackYears/PackYears_WC/packyears_smk_wc_post_20082021.tiff", units="in", width=10, height=10, res=300)
mr_plots(dat_1_post)
dev.off()
