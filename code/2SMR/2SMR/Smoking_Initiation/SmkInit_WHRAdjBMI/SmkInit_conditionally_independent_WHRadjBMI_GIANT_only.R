...
whradjbmi_match_by_rsid <- whradjbmi[which(whradjbmi$MarkerName%in%SMK_gw$rsID),] #173/378

#Do we have a triallelic SNP that is chosen twice?

dupl <- whradjbmi_match_by_rsid$MarkerName[which(duplicated(whradjbmi_match_by_rsid$MarkerName) == TRUE)]

whradjbmi_match_by_rsid[which(whradjbmi_match_by_rsid$MarkerName == dupl),]

#Empty data.table (0 rows and 12 cols): MarkerName,Chr,Pos,Allele1,Allele2,FreqAllele1HapMapCEU...

summary(whradjbmi_match_by_rsid$FreqAllele1HapMapCEU) #all good. No need to do anything else
...
length(which(whradjbmi_match_by_rsid$chr_pos_37%in%SMK_gw$chr_pos_37)) #175/378. Exactly the same as with RSID.
...
summary(whradjbmi_match_by_rsid$N) #no NAs, all good.
#summary(whr_match_by_rsid$INFO) #no info in GIANT data here.
summary(whradjbmi_match_by_rsid$FreqAllele1HapMapCEU) #perfect.
...
SMK_gw_missing <- SMK_gw[which(!(SMK_gw$chr_pos_37%in%whradjbmi_match_by_rsid$chr_pos_37)),]

#We are gonna save these SNPs for checking the rest of the traits.

fwrite(SMK_gw_missing, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/2SMR/Smoking_Initiation/SmkInit_WHRAdjBMI/proxies/conditionally_missing_snps.txt", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
whradjbmi_outcome <- whradjbmi #173/173
...
dat_1_pre <- harmonise_data(SMK_gw, whradjbmi_outcome, action = 3) #173: perfect.
...
dat_1_filt_1 <- dat_1_filt[which(dat_1_filt$steiger_dir == TRUE),] #130/173. Not bad!!
...
fwrite(dat_1_filt, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/Mediation_analysis/output/1_curated_data/smoking_initiation_whradjbmi_curated.txt")

#################################
#We do a small check on the data#
#################################
...
#1 smoking initiation  WHRadjBMI WHRadjBMI smoking initiation                  MR Egger  130  0.131439541 0.09220260 0.1564320
#2 smoking initiation  WHRadjBMI WHRadjBMI smoking initiation           Weighted median  130  0.032334265 0.03026443 0.2853439
#3 smoking initiation  WHRadjBMI WHRadjBMI smoking initiation Inverse variance weighted  130  0.030969250 0.02156752 0.1510248
#4 smoking initiation  WHRadjBMI WHRadjBMI smoking initiation               Simple mode  130 -0.008493181 0.08682830 0.9222304
#5 smoking initiation  WHRadjBMI WHRadjBMI smoking initiation             Weighted mode  130 -0.015271013 0.08166816 0.8519634
...
#1  Egger fixed effects -0.001937679 0.001286002 -0.004458196 0.0005828385 0.1318756
#2 Egger random effects -0.001937679 0.001286002 -0.004458196 0.0005828385 0.9340622

#[[1]]$Q
#Method         Q  df         P
#1   Q_ivw 72.076157 129 0.9999877
#2 Q_egger 70.820050 128 0.9999901
#3  Q_diff  1.256107   1 0.2623893
...
#1 Rucker  130 0.03096925 0.01612134 -0.0006279935 0.06256649 0.05473047
...
tiff("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/2SMR/Smoking_Initiation/SmkInit_WHRAdjBMI/SMK_conditionally_independent_WHRadjBMI_GIANT_all.tiff ", units="in", width=10, height=10, res=300)
mr_plots(dat_1_filt)
dev.off()
