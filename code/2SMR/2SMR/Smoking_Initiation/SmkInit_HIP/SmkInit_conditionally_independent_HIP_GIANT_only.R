...
#This is the 2SMR code to run Lifetime smoking with hip (GIANT)
...
hip_match_by_rsid <- hip[which(hip$MarkerName%in%SMK_gw$rsID),] #173/378

#Do we have a triallelic SNP that is chosen twice?

dupl <- hip_match_by_rsid$MarkerName[which(duplicated(hip_match_by_rsid$MarkerName) == TRUE)]

hip_match_by_rsid[which(hip_match_by_rsid$MarkerName == dupl),]

#Empty data.table (0 rows and 12 cols): MarkerName,Chr,Pos,Allele1,Allele2,FreqAllele1HapMapCEU...

summary(hip_match_by_rsid$FreqAllele1HapMapCEU) #all good. No need to do anything else
...
length(which(hip_match_by_rsid$chr_pos_37%in%SMK_gw$chr_pos_37)) #173/378. Exactly the same as with RSID.
...
summary(hip_match_by_rsid$N) #no NAs, all good.
#summary(hip_match_by_rsid$INFO) #no info in GIANT data here.
summary(hip_match_by_rsid$FreqAllele1HapMapCEU) #perfect.
...
SMK_gw_missing <- SMK_gw[which(!(SMK_gw$chr_pos_37%in%hip_match_by_rsid$chr_pos_37)),]
...
#whr_proxies <- hip[which(hip$chr_pos_37%in%proxies_chr_pos),] #695/1781. Makes sense
...
#SMK_gw_ind_not_missing <- SMK_gw_ind[which(SMK_gw_ind$chr_pos_37%in%hip_match_by_rsid$chr_pos_37),]
...
dat_1_pre <- harmonise_data(SMK_gw, hip_outcome, action = 3) #173: perfect.
...
dat_1_filt_1 <- dat_1_filt[which(dat_1_filt$steiger_dir == TRUE),] #116/163. Not bad!!
...
fwrite(dat_1_filt, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/Mediation_analysis/output/1_curated_data/smoking_initiation_hip_curated.txt")
...
#1 smoking initiation        hip     hip smoking initiation                  MR Egger  116 -0.01313239 0.10891927 0.9042439
#2 smoking initiation        hip     hip smoking initiation           Weighted median  116  0.03207951 0.03442909 0.3514627
#3 smoking initiation        hip     hip smoking initiation Inverse variance weighted  116  0.01681794 0.02402127 0.4838480
#4 smoking initiation        hip     hip smoking initiation               Simple mode  116  0.08415666 0.09252705 0.3649697
#5 smoking initiation        hip     hip smoking initiation             Weighted mode  116  0.05120812 0.08880278 0.5653030
...
#1  Egger fixed effects 0.0005678632 0.001582248 -0.002533285 0.003669011 0.7196725
#2 Egger random effects 0.0005678632 0.001582248 -0.002533285 0.003669011 0.3598362

#[[1]]$Q
#Method           Q  df         P
#1   Q_ivw 70.42147164 115 0.9996539
#2 Q_egger 70.34199338 114 0.9995631
#3  Q_diff  0.07947826   1 0.7780057
...
#1 Rucker  116 0.01681794 0.01879748 -0.02002444 0.05366032 0.3709522
...
tiff("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/2SMR/Smoking_Initiation/SmkInit_HIP/SMK_conditionally_independent_HIP_GIANT_all.tiff ", units="in", width=10, height=10, res=300)
mr_plots(dat_1_filt)
dev.off()
