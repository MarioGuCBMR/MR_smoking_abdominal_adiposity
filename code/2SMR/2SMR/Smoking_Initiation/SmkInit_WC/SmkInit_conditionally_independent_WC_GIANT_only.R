...
wc_match_by_rsid <- wc[which(wc$MarkerName%in%SMK_gw$rsID),] #173/378

#Do we have a triallelic SNP that is chosen twice?

dupl <- wc_match_by_rsid$MarkerName[which(duplicated(wc_match_by_rsid$MarkerName) == TRUE)]

wc_match_by_rsid[which(wc_match_by_rsid$MarkerName == dupl),]

#Empty data.table (0 rows and 12 cols): MarkerName,Chr,Pos,Allele1,Allele2,FreqAllele1HapMapCEU...

summary(wc_match_by_rsid$FreqAllele1HapMapCEU) #all good. No need to do anything else
...
length(which(wc_match_by_rsid$chr_pos_37%in%SMK_gw$chr_pos_37)) #175/378. Exactly the same as with RSID.
...
summary(wc_match_by_rsid$N) #no NAs, all good.
#summary(whr_match_by_rsid$INFO) #no info in GIANT data here.
summary(wc_match_by_rsid$FreqAllele1HapMapCEU) #perfect.
...
SMK_gw_missing <- SMK_gw[which(!(SMK_gw$chr_pos_37%in%wc_match_by_rsid$chr_pos_37)),]
...
dat_1_pre <- harmonise_data(SMK_gw, wc_outcome, action = 3) #175: perfect.
...
dat_1_filt_1 <- dat_1_filt[which(dat_1_filt$steiger_dir == TRUE),] #115/163. Not bad!!
...
fwrite(dat_1_filt, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/Mediation_analysis/output/1_curated_data/smoking_initiation_wc_curated.txt")
...
#1 smoking initiation         WC      WC smoking initiation                  MR Egger  115 0.06841297 0.10485117 0.515418677
#2 smoking initiation         WC      WC smoking initiation           Weighted median  115 0.09283963 0.03375019 0.005945203
#3 smoking initiation         WC      WC smoking initiation Inverse variance weighted  115 0.05686042 0.02334554 0.014867091
#4 smoking initiation         WC      WC smoking initiation               Simple mode  115 0.11948575 0.09197792 0.196541176
#5 smoking initiation         WC      WC smoking initiation             Weighted mode  115 0.11054107 0.08760197 0.209576269
...
#1  Egger fixed effects -0.0002180916 0.001489088 -0.003136649 0.002700466 0.8835584
#2 Egger random effects -0.0002180916 0.001489088 -0.003136649 0.002700466 0.5582208

#[[1]]$Q
#Method           Q  df         P
#1   Q_ivw 67.30004511 114 0.9998482
#2 Q_egger 67.28727215 113 0.9998008
#3  Q_diff  0.01277297   1 0.9100166
...
#1 Rucker  115 0.05686042 0.01793739 0.02170378 0.09201705 0.001524713
...
tiff("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/2SMR/2SMR/Smoking_Initiation/SmkInit_WC/SMK_conditionally_independent_WC_GIANT_all.tiff ", units="in", width=10, height=10, res=300)
mr_plots(dat_1_filt)
dev.off()
