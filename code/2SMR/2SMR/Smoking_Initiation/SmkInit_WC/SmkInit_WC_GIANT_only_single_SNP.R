...
wc_2_rs1051730 <- wc[which(wc$MarkerName%in%rs1051730_exp$RSID),] #gotcha
wc_2_rs16969968 <- wc[which(wc$MarkerName%in%rs16969968_exp$RSID),] #gotcha
wc_2_rs1317286 <- wc[which(wc$MarkerName%in%rs1317286_exp$RSID),] #gotcha
...
dat_1_rs1051730 <- harmonise_data(rs1051730_exp_clean, wc_outcome_rs1051730, action = 3) 
dat_1_rs16969968 <- harmonise_data(rs16969968_exp_clean, wc_outcome_rs16969968, action = 3) 
dat_1_rs1317286 <- harmonise_data(rs1317286_exp_clean, wc_outcome_rs1317286, action = 3) 
...
#1         SMK         wc      wc      SMK Wald ratio    1 0.5650769 0.3507374 0.1071555

mr(dat_1_rs16969968)

#id.exposure id.outcome outcome exposure     method nsnp         b        se       pval
#1         SMK         wc      wc      SMK Wald ratio    1 0.6451935 0.3746285 0.08502927

mr(dat_1_rs1317286)

#id.exposure id.outcome outcome exposure     method nsnp         b        se      pval
#1         SMK         wc      wc      SMK Wald ratio    1 0.5399957 0.4584869 0.2388852
