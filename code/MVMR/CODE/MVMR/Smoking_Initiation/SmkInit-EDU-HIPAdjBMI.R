...
HIPAdjBMI_match <- HIPAdjBMI[which(HIPAdjBMI$chr_pos_37%in%SMK_edu_df$chr_pos),] #137
check <- HIPAdjBMI[which(HIPAdjBMI$MarkerName%in%SMK_edu_df$SNP),] #137 too.

#That means that we need to run proxies for about 140 SNPs.
#Let's go:

missings_snps <- SMK_edu_df[which(!(SMK_edu_df$chr_pos%in%HIPAdjBMI$chr_pos_37)),]
...
#Since WHR, whr, HIPAdjBMI and HIPAdjBMIAdjBMI come from the same study, the possiblity that the mising SNPs are
...
HIPAdjBMI_proxies_match_smk <- HIPAdjBMI[which(HIPAdjBMI$chr_pos_37%in%proxies_df_smk$Coord),] #332
HIPAdjBMI_proxies_match_edu <- HIPAdjBMI[which(HIPAdjBMI$chr_pos_37%in%proxies_df_edu$Coord),] #1752

#This is going to be always the final threshold since HIPAdjBMI is a bottleneck. Hence...
#we are gonna use these SNPs to match the rest:

SMK_proxies_match <- SMK[which(SMK$chr_pos%in%HIPAdjBMI_proxies_match_smk$chr_pos_37),] #328. Almost all of them!
...
edu_proxies_match <- extract_outcome_data(
  snps = HIPAdjBMI_proxies_match_edu$MarkerName,
...
SMK_edu_df_not_missing <- SMK_edu_df[which(SMK_edu_df$chr_pos%in%HIPAdjBMI$chr_pos_37),]
...
#2. Then we checked how many of those are in HIPAdjBMI because it is the bottleneck.
...
HIPAdjBMI_outcome <- HIPAdjBMI[which(HIPAdjBMI$MarkerName%in%SMK_edu_independent$SNP),] #229/229

length(which(HIPAdjBMI_outcome$MarkerName%in%SMK_edu_independent$SNP)) #all of them!

summary(HIPAdjBMI_outcome$FreqAllele1HapMapCEU) #all good.

#1. we order the SNPs:

HIPAdjBMI_outcome <- HIPAdjBMI_outcome[order(match(HIPAdjBMI_outcome$MarkerName, SMK_edu_independent$SNP)),]

#Let's check this:

length(which(HIPAdjBMI_outcome$MarkerName != SMK_edu_independent$SNP)) #0
length(which(HIPAdjBMI_outcome$MarkerName == SMK_edu_independent$SNP)) #229

length(which(HIPAdjBMI_outcome$chr_pos_37 != SMK_edu_independent$chr_pos)) #0
length(which(HIPAdjBMI_outcome$MarkerName == SMK_edu_independent$SNP)) #229
...
HIPAdjBMI_outcome$final_a1 <- ifelse(HIPAdjBMI_outcome$Allele1 != SMK_edu_independent$effect_allele, HIPAdjBMI_outcome$Allele2, HIPAdjBMI_outcome$Allele1)
HIPAdjBMI_outcome$final_a2 <- ifelse(HIPAdjBMI_outcome$Allele1 != SMK_edu_independent$effect_allele, HIPAdjBMI_outcome$Allele1, HIPAdjBMI_outcome$Allele2)
HIPAdjBMI_outcome$final_beta <- ifelse(HIPAdjBMI_outcome$Allele1 != SMK_edu_independent$effect_allele, HIPAdjBMI_outcome$b*(-1), HIPAdjBMI_outcome$b)
HIPAdjBMI_outcome$final_eaf <- ifelse(HIPAdjBMI_outcome$Allele1 != SMK_edu_independent$effect_allele, 1-HIPAdjBMI_outcome$FreqAllele1HapMapCEU, HIPAdjBMI_outcome$FreqAllele1HapMapCEU)

#Let's check:

length(which(HIPAdjBMI_outcome$final_a1 != SMK_edu_independent$effect_allele)) #0
length(which(HIPAdjBMI_outcome$final_a1 == SMK_edu_independent$effect_allele)) #229

length(which(HIPAdjBMI_outcome$final_a2 != SMK_edu_independent$other_allele)) #0
length(which(HIPAdjBMI_outcome$final_a2 == SMK_edu_independent$other_allele)) #229

head(HIPAdjBMI_outcome) #worked like a charm. 
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Smoking_Initiation/OUTPUT/HIPAdjBMI/"

mvmr_Mario(SMK_edu_independent, HIPAdjBMI_outcome, path_)
