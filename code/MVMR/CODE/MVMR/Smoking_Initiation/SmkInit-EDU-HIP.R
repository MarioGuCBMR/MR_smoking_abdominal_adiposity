...
hip_match <- hip[which(hip$chr_pos_37%in%SMK_edu_df$chr_pos),] #137
check <- hip[which(hip$MarkerName%in%SMK_edu_df$SNP),] #137 too.

#That means that we need to run proxies for about 140 SNPs.
#Let's go:

missings_snps <- SMK_edu_df[which(!(SMK_edu_df$chr_pos%in%hip$chr_pos_37)),]
...
#Since WHR, whr, hip and hipAdjBMI come from the same study, the possiblity that the mising SNPs are
#the same is really high.

missings_snps_2 <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Smoking_Initiation/proxies/missing_snps.txt")

length(which(missings_snps$SNP%in%missings_snps_2$SNP)) #all of them, of course.

#####################################
#Great, now let's detect the proxies#
#####################################

#NO NEED TO GET PROXIES AGAIN. THE MISSING SNPS ARE THE SAME.
...
#########################################
#Let's get those that are present in hip#
#########################################

hip_proxies_match_smk <- hip[which(hip$chr_pos_37%in%proxies_df_smk$Coord),] #332
hip_proxies_match_edu <- hip[which(hip$chr_pos_37%in%proxies_df_edu$Coord),] #1752

#This is going to be always the final threshold since hip is a bottleneck. Hence...
#we are gonna use these SNPs to match the rest:

SMK_proxies_match <- SMK[which(SMK$chr_pos%in%hip_proxies_match_smk$chr_pos_37),] #328. Almost all of them!
...
edu_proxies_match <- extract_outcome_data(
  snps = hip_proxies_match_edu$MarkerName,
...
SMK_edu_df_not_missing <- SMK_edu_df[which(SMK_edu_df$chr_pos%in%hip$chr_pos_37),]
...
#2. Then we checked how many of those are in hip because it is the bottleneck.
...
hip_outcome <- hip[which(hip$MarkerName%in%SMK_edu_independent$SNP),] #229/229

length(which(hip_outcome$MarkerName%in%SMK_edu_independent$SNP)) #all of them!

summary(hip_outcome$FreqAllele1HapMapCEU) #all good.

#1. we order the SNPs:

hip_outcome <- hip_outcome[order(match(hip_outcome$MarkerName, SMK_edu_independent$SNP)),]

#Let's check this:

length(which(hip_outcome$MarkerName != SMK_edu_independent$SNP)) #0
length(which(hip_outcome$MarkerName == SMK_edu_independent$SNP)) #229

length(which(hip_outcome$chr_pos_37 != SMK_edu_independent$chr_pos)) #0
length(which(hip_outcome$MarkerName == SMK_edu_independent$SNP)) #229
...
hip_outcome$final_a1 <- ifelse(hip_outcome$Allele1 != SMK_edu_independent$effect_allele, hip_outcome$Allele2, hip_outcome$Allele1)
hip_outcome$final_a2 <- ifelse(hip_outcome$Allele1 != SMK_edu_independent$effect_allele, hip_outcome$Allele1, hip_outcome$Allele2)
hip_outcome$final_beta <- ifelse(hip_outcome$Allele1 != SMK_edu_independent$effect_allele, hip_outcome$b*(-1), hip_outcome$b)
hip_outcome$final_eaf <- ifelse(hip_outcome$Allele1 != SMK_edu_independent$effect_allele, 1-hip_outcome$FreqAllele1HapMapCEU, hip_outcome$FreqAllele1HapMapCEU)

#Let's check:

length(which(hip_outcome$final_a1 != SMK_edu_independent$effect_allele)) #1
length(which(hip_outcome$final_a1 == SMK_edu_independent$effect_allele)) #228

length(which(hip_outcome$final_a2 != SMK_edu_independent$other_allele)) #1
length(which(hip_outcome$final_a2 == SMK_edu_independent$other_allele)) #228

#There is one that is not working because they are in different strands.
#No worries! 

which(hip_outcome$final_a1 != SMK_edu_independent$effect_allele) #98

hip_outcome[98,]
SMK_edu_independent[98,]

#They are the same SNP, but..., they are on different strands!!

#Really easy, now we can do the following:

SMK_edu_independent$effect_allele[98] <- "T"
SMK_edu_independent$other_allele[98] <- "G"

#No need to change anything, because the effect and other allele match!

#Let's repeat this then:
...
length(which(hip_outcome$final_a1 != SMK_edu_independent$effect_allele)) #0
length(which(hip_outcome$final_a1 == SMK_edu_independent$effect_allele)) #229

length(which(hip_outcome$final_a2 != SMK_edu_independent$other_allele)) #0
length(which(hip_outcome$final_a2 == SMK_edu_independent$other_allele)) #229
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Smoking_Initiation/OUTPUT/HIP/"

mvmr_Mario(SMK_edu_independent, hip_outcome, path_)
