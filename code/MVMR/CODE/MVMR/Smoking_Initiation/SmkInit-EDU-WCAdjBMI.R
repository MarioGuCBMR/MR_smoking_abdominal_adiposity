...
WCAdjBMI_match <- WCAdjBMI[which(WCAdjBMI$chr_pos_37%in%SMK_edu_df$chr_pos),] #137
check <- WCAdjBMI[which(WCAdjBMI$MarkerName%in%SMK_edu_df$SNP),] #137 too.

#That means that we need to run proxies for about 140 SNPs.
#Let's go:

missings_snps <- SMK_edu_df[which(!(SMK_edu_df$chr_pos%in%WCAdjBMI$chr_pos_37)),]
...
#Since WHR, whr, WCAdjBMI and WCAdjBMIAdjBMI come from the same study, the possiblity that the mising SNPs are
...
WCAdjBMI_proxies_match_smk <- WCAdjBMI[which(WCAdjBMI$chr_pos_37%in%proxies_df_smk$Coord),] #332
WCAdjBMI_proxies_match_edu <- WCAdjBMI[which(WCAdjBMI$chr_pos_37%in%proxies_df_edu$Coord),] #1752

#This is going to be always the final threshold since WCAdjBMI is a bottleneck. Hence...
#we are gonna use these SNPs to match the rest:

SMK_proxies_match <- SMK[which(SMK$chr_pos%in%WCAdjBMI_proxies_match_smk$chr_pos_37),] #328. Almost all of them!
...
edu_proxies_match <- extract_outcome_data(
  snps = WCAdjBMI_proxies_match_edu$MarkerName,
...
SMK_edu_df_not_missing <- SMK_edu_df[which(SMK_edu_df$chr_pos%in%WCAdjBMI$chr_pos_37),]
...
#2. Then we checked how many of those are in WCAdjBMI because it is the bottleneck.
...
WCAdjBMI_outcome <- WCAdjBMI[which(WCAdjBMI$MarkerName%in%SMK_edu_independent$SNP),] #229/229

length(which(WCAdjBMI_outcome$MarkerName%in%SMK_edu_independent$SNP)) #all of them!

summary(WCAdjBMI_outcome$FreqAllele1HapMapCEU) #all good.

#1. we order the SNPs:

WCAdjBMI_outcome <- WCAdjBMI_outcome[order(match(WCAdjBMI_outcome$MarkerName, SMK_edu_independent$SNP)),]

#Let's check this:

length(which(WCAdjBMI_outcome$MarkerName != SMK_edu_independent$SNP)) #0
length(which(WCAdjBMI_outcome$MarkerName == SMK_edu_independent$SNP)) #229

length(which(WCAdjBMI_outcome$chr_pos_37 != SMK_edu_independent$chr_pos)) #0
length(which(WCAdjBMI_outcome$MarkerName == SMK_edu_independent$SNP)) #229
...
WCAdjBMI_outcome$final_a1 <- ifelse(WCAdjBMI_outcome$Allele1 != SMK_edu_independent$effect_allele, WCAdjBMI_outcome$Allele2, WCAdjBMI_outcome$Allele1)
WCAdjBMI_outcome$final_a2 <- ifelse(WCAdjBMI_outcome$Allele1 != SMK_edu_independent$effect_allele, WCAdjBMI_outcome$Allele1, WCAdjBMI_outcome$Allele2)
WCAdjBMI_outcome$final_beta <- ifelse(WCAdjBMI_outcome$Allele1 != SMK_edu_independent$effect_allele, WCAdjBMI_outcome$b*(-1), WCAdjBMI_outcome$b)
WCAdjBMI_outcome$final_eaf <- ifelse(WCAdjBMI_outcome$Allele1 != SMK_edu_independent$effect_allele, 1-WCAdjBMI_outcome$FreqAllele1HapMapCEU, WCAdjBMI_outcome$FreqAllele1HapMapCEU)

#Let's check:

length(which(WCAdjBMI_outcome$final_a1 != SMK_edu_independent$effect_allele)) #0
length(which(WCAdjBMI_outcome$final_a1 == SMK_edu_independent$effect_allele)) #229

length(which(WCAdjBMI_outcome$final_a2 != SMK_edu_independent$other_allele)) #0
length(which(WCAdjBMI_outcome$final_a2 == SMK_edu_independent$other_allele)) #229

head(WCAdjBMI_outcome) #worked like a charm. 
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Smoking_Initiation/OUTPUT/WCAdjBMI/"

mvmr_Mario(SMK_edu_independent, WCAdjBMI_outcome, path_)
