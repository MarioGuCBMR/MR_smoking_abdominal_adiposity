...
whr_match <- whr[which(whr$chr_pos_37%in%SMK_edu_df$chr_pos),] #137
check <- whr[which(whr$MarkerName%in%SMK_edu_df$SNP),] #137 too.

#That means that we need to run proxies for about 140 SNPs.
#Let's go:

missings_snps <- SMK_edu_df[which(!(SMK_edu_df$chr_pos%in%whr$chr_pos_37)),]
...
#Since WHR, whr, WC and WCAdjBMI come from the same study, the possiblity that the mising SNPs are
...
whr_proxies_match_smk <- whr[which(whr$chr_pos_37%in%proxies_df_smk$Coord),] #332
whr_proxies_match_edu <- whr[which(whr$chr_pos_37%in%proxies_df_edu$Coord),] #1752
...
SMK_proxies_match <- SMK[which(SMK$chr_pos%in%whr_proxies_match_smk$chr_pos_37),] #328. Almost all of them!
...
SMK_edu_df_not_missing <- SMK_edu_df[which(SMK_edu_df$chr_pos%in%whr$chr_pos_37),]
...
whr_outcome <- whr[which(whr$MarkerName%in%SMK_edu_independent$SNP),] #229/229

length(which(whr_outcome$MarkerName%in%SMK_edu_independent$SNP)) #all of them!

summary(whr_outcome$FreqAllele1HapMapCEU) #all good.

#1. we order the SNPs:

whr_outcome <- whr_outcome[order(match(whr_outcome$MarkerName, SMK_edu_independent$SNP)),]

#Let's check this:

length(which(whr_outcome$MarkerName != SMK_edu_independent$SNP)) #0
length(which(whr_outcome$MarkerName == SMK_edu_independent$SNP)) #229

length(which(whr_outcome$chr_pos_37 != SMK_edu_independent$chr_pos)) #0
length(which(whr_outcome$MarkerName == SMK_edu_independent$SNP)) #229
...
whr_outcome$final_a1 <- ifelse(whr_outcome$Allele1 != SMK_edu_independent$effect_allele, whr_outcome$Allele2, whr_outcome$Allele1)
whr_outcome$final_a2 <- ifelse(whr_outcome$Allele1 != SMK_edu_independent$effect_allele, whr_outcome$Allele1, whr_outcome$Allele2)
whr_outcome$final_beta <- ifelse(whr_outcome$Allele1 != SMK_edu_independent$effect_allele, whr_outcome$b*(-1), whr_outcome$b)
whr_outcome$final_eaf <- ifelse(whr_outcome$Allele1 != SMK_edu_independent$effect_allele, 1-whr_outcome$FreqAllele1HapMapCEU, whr_outcome$FreqAllele1HapMapCEU)

#Let's check:

length(which(whr_outcome$final_a1 != SMK_edu_independent$effect_allele)) #0
length(which(whr_outcome$final_a1 == SMK_edu_independent$effect_allele)) #229

length(which(whr_outcome$final_a2 != SMK_edu_independent$other_allele)) #0
length(which(whr_outcome$final_a2 == SMK_edu_independent$other_allele)) #229
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Smoking_Initiation/OUTPUT/WHR/"

mvmr_Mario(SMK_edu_independent, whr_outcome, path_)
