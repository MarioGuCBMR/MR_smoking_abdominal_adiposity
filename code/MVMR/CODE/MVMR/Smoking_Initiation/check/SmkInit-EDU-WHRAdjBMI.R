...
#Also we are gonna work without the functions, just to see if the way that we have built them makes sense.
#I think that there is no need to worry, but it is better doing it this way for the time being.
...
SMK$chr_pos <- paste("chr", SMK$CHROM, ":", SMK$POS, sep = "")
summary(SMK$INFO) #0.3 is minumum as described by README.

SMK_edu_df <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/Curated_data/SmkInit_EDU_Risk_Factors.txt") #275

##############################
#Let's do some checks here...#
##############################

length(which(SMK_edu_df$pval.smk < 0.00000005)) #59
length(which(SMK_edu_df$pval.edu < 0.00000005)) #222

#this seems suspicious...

SMK_edu_df[which.min(SMK_edu_df$pval.smk),]

SMK[which(SMK$RSID == "rs7938812"),]
...
whradjbmi_match <- whradjbmi[which(whradjbmi$chr_pos_37%in%SMK_edu_df$chr_pos),] #137
check <- whradjbmi[which(whradjbmi$MarkerName%in%SMK_edu_df$SNP),] #137 too.

#That means that we need to run proxies for about 140 SNPs.
#Let's go:

missings_snps <- SMK_edu_df[which(!(SMK_edu_df$chr_pos%in%whradjbmi$chr_pos_37)),]

#Let's check which of the SNPs are from which dataframe.

missing_smk <- missings_snps[which(missings_snps$source == "Smoking Initiation"),] #20/138
missing_edu <- missings_snps[which(missings_snps$source == "Education"),] #118/138

#This sums 138. Now is perfect.
...
fwrite(missings_snps, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Smoking_Initiation/proxies/missing_snps.txt")

missings_snps <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Smoking_Initiation/proxies/missing_snps.txt")
...
setwd("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Smoking_Initiation/proxies/")
...
proxies_df <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Smoking_Initiation/proxies/combined_query_snp_list.txt", fill = TRUE)

query_snps <- proxies_df[which(proxies_df$query_snp == proxies_df$Coord & proxies_df$Distance == 0 & proxies_df$R2 == 1),] #138 perfect.
...
proxies_df_clean_curated <- proxies_df_clean[which(proxies_df_clean$R2 > 0.8),] #7139

#Now we are gonna divide the SNPs depending on the query, whether it was from lifetime or it was from education.

proxies_df_smk <- proxies_df_clean_curated[which(proxies_df_clean_curated$query_snp%in%missing_smk$chr_pos),] #811
proxies_df_edu <- proxies_df_clean_curated[which(proxies_df_clean_curated$query_snp%in%missing_edu$chr_pos),] #6328

###############################################
#Let's get those that are present in WHRAdjBMI#
###############################################

whradjbmi_proxies_match_smk <- whradjbmi[which(whradjbmi$chr_pos_37%in%proxies_df_smk$Coord),] #332
whradjbmi_proxies_match_edu <- whradjbmi[which(whradjbmi$chr_pos_37%in%proxies_df_edu$Coord),] #1752
...
SMK_proxies_match <- SMK[which(SMK$chr_pos%in%whradjbmi_proxies_match_smk$chr_pos_37),] #328. Almost all of them!

summary(SMK_proxies_match$AF) #perfect
summary(SMK_proxies_match$INFO) #perfect
...
edu_proxies_match <- extract_outcome_data(
  snps = whradjbmi_proxies_match_edu$MarkerName,
...
independent_set_smk <- SMK_proxies_match %>%
  select(RSID, PVALUE)

colnames(independent_set_smk) <- c("rsid", "pval")
...
SMK_edu_df_not_missing <- SMK_edu_df[which(SMK_edu_df$chr_pos%in%whradjbmi$chr_pos_37),]

#And here we divide too, by those that are genome-wide significant for each of the SNPs.

SMK_only_not_missing <- SMK_edu_df_not_missing[which(SMK_edu_df_not_missing$source == "Smoking Initiation"),] #33/137
edu_only_not_missing <- SMK_edu_df_not_missing[which(SMK_edu_df_not_missing$source == "Education"),] #104/137
...
independent_set_smk_not_missing <- SMK_only_not_missing %>%
  select(SNP, pval.smk)

colnames(independent_set_smk_not_missing) <- c("rsid", "pval")

independent_final_set <- rbind(independent_set_smk, independent_set_edu, independent_set_smk_not_missing, independent_set_edu_not_missing) #A total of 1911 SNPs.

#Do we have any duplicates here??

independent_final_set[which(duplicated(independent_final_set$rsid) == TRUE),] #the same as always from EDU. We can erase in the same way as always.

independent_final_set <- independent_final_set[order(independent_final_set$pval),]

independent_final_set <- independent_final_set[which(duplicated(independent_final_set$rsid) == FALSE),]
...
#We get 232, but we have to be wary since there are duplicates and maybe palindromic ones.
...
SMK_proxies_match <- SMK[which(SMK$RSID%in%final_set_of_snps$rsid),] #232

edu_proxies_match <- extract_outcome_data(
  snps = SMK_proxies_match$RSID,
  outcomes = 'ukb-b-16489',
  proxies = FALSE
)
...
#And combine it with the original SMK_edu data.

colnames(SMK_proxies_match) <- c("chr.exposure", "pos.exposure", "SNP", "other_allele.exposure", "effect_allele.exposure", 
                                 ...
                                 "info.exposure", "chr_pos")

SMK_proxies_match$id.exposure <- "SMK"
SMK_proxies_match$exposure <- "SMK"

dat_1_smk2edu_proxies <- harmonise_data(SMK_proxies_match, edu_proxies_match, action = 3) #Note it removes the wrong triallelic combinations.
...
#With this version we get 229 SNPs...
...
SMK_edu_independent <- dat_1_smk2edu_proxies_clean
...
whradjbmi_outcome <- whradjbmi[which(whradjbmi$MarkerName%in%SMK_edu_independent$SNP),] #229/229

length(which(whradjbmi_outcome$MarkerName%in%SMK_edu_independent$SNP)) #all of them!

#1. we order the SNPs:

whradjbmi_outcome <- whradjbmi_outcome[order(match(whradjbmi_outcome$MarkerName, SMK_edu_independent$SNP)),]

#Let's check this:

length(which(whradjbmi_outcome$MarkerName != SMK_edu_independent$SNP)) #0
length(which(whradjbmi_outcome$MarkerName == SMK_edu_independent$SNP)) #233

length(which(whradjbmi_outcome$chr_pos_37 != SMK_edu_independent$chr_pos)) #0
length(which(whradjbmi_outcome$MarkerName == SMK_edu_independent$SNP)) #233
...
whradjbmi_outcome$final_a1 <- ifelse(whradjbmi_outcome$Allele1 != SMK_edu_independent$effect_allele, whradjbmi_outcome$Allele2, whradjbmi_outcome$Allele1)
whradjbmi_outcome$final_a2 <- ifelse(whradjbmi_outcome$Allele1 != SMK_edu_independent$effect_allele, whradjbmi_outcome$Allele1, whradjbmi_outcome$Allele2)
whradjbmi_outcome$final_beta <- ifelse(whradjbmi_outcome$Allele1 != SMK_edu_independent$effect_allele, whradjbmi_outcome$b*(-1), whradjbmi_outcome$b)
whradjbmi_outcome$final_eaf <- ifelse(whradjbmi_outcome$Allele1 != SMK_edu_independent$effect_allele, 1-whradjbmi_outcome$FreqAllele1HapMapCEU, whradjbmi_outcome$FreqAllele1HapMapCEU)

#Let's check:

length(which(whradjbmi_outcome$final_a1 != SMK_edu_independent$effect_allele)) #0
length(which(whradjbmi_outcome$final_a1 == SMK_edu_independent$effect_allele)) #229

length(which(whradjbmi_outcome$final_a2 != SMK_edu_independent$other_allele)) #0
length(which(whradjbmi_outcome$final_a2 == SMK_edu_independent$other_allele)) #229
...
#Let's take my function and do it step by step here to see if there is any mistake.
#The function is so long that I am afraid that might have happened.

#Nonetheless, we are gonna just do it once: for phenotypic correlation -0.15.

dat = SMK_edu_independent
outcome <- whradjbmi_outcome
...
#  304.8902 on 226 DF , p-value: 0.000361812
...
#exposure1  0.0599779 0.02688674  2.230762 0.026674980
#exposure2 -0.1898672 0.05870895 -3.234042 0.001402262
...
#Q-Statistic for instrument validity:
#  164.6681 on 204 DF , p-value: 0.980064
...
#Estimate Std. Error   t value     Pr(>|t|)
#exposure1  0.08620522 0.02210053  3.900596 0.0001300556
#exposure2 -0.14650734 0.04823298 -3.037493 0.0026955726

#####################################################
#The results are exactly the same. I saw no error :)#
#####################################################