...
wc_match <- wc[which(wc$chr_pos_37%in%SMK_edu_df$chr_pos),] #137
check <- wc[which(wc$MarkerName%in%SMK_edu_df$SNP),] #137 too.

#That means that we need to run proxies for about 140 SNPs.
#Let's go:

missings_snps <- SMK_edu_df[which(!(SMK_edu_df$chr_pos%in%wc$chr_pos_37)),]
...
wc_proxies_match_smk <- wc[which(wc$chr_pos_37%in%proxies_df_smk$Coord),] #332
wc_proxies_match_edu <- wc[which(wc$chr_pos_37%in%proxies_df_edu$Coord),] #1752
...
SMK_proxies_match <- SMK[which(SMK$chr_pos%in%wc_proxies_match_smk$chr_pos_37),] #328. Almost all of them!
...
edu_proxies_match <- extract_outcome_data(
  snps = wc_proxies_match_edu$MarkerName,
...
SMK_edu_df_not_missing <- SMK_edu_df[which(SMK_edu_df$chr_pos%in%wc$chr_pos_37),]
...
wc_outcome <- wc[which(wc$MarkerName%in%SMK_edu_independent$SNP),] #229/229

length(which(wc_outcome$MarkerName%in%SMK_edu_independent$SNP)) #all of them!

summary(wc_outcome$FreqAllele1HapMapCEU) #all good.

#1. we order the SNPs:

wc_outcome <- wc_outcome[order(match(wc_outcome$MarkerName, SMK_edu_independent$SNP)),]

#Let's check this:

length(which(wc_outcome$MarkerName != SMK_edu_independent$SNP)) #0
length(which(wc_outcome$MarkerName == SMK_edu_independent$SNP)) #229

length(which(wc_outcome$chr_pos_37 != SMK_edu_independent$chr_pos)) #0
length(which(wc_outcome$MarkerName == SMK_edu_independent$SNP)) #229
...
wc_outcome$final_a1 <- ifelse(wc_outcome$Allele1 != SMK_edu_independent$effect_allele, wc_outcome$Allele2, wc_outcome$Allele1)
wc_outcome$final_a2 <- ifelse(wc_outcome$Allele1 != SMK_edu_independent$effect_allele, wc_outcome$Allele1, wc_outcome$Allele2)
wc_outcome$final_beta <- ifelse(wc_outcome$Allele1 != SMK_edu_independent$effect_allele, wc_outcome$b*(-1), wc_outcome$b)
wc_outcome$final_eaf <- ifelse(wc_outcome$Allele1 != SMK_edu_independent$effect_allele, 1-wc_outcome$FreqAllele1HapMapCEU, wc_outcome$FreqAllele1HapMapCEU)

#Let's check:

length(which(wc_outcome$final_a1 != SMK_edu_independent$effect_allele)) #0
length(which(wc_outcome$final_a1 == SMK_edu_independent$effect_allele)) #229

length(which(wc_outcome$final_a2 != SMK_edu_independent$other_allele)) #0
length(which(wc_outcome$final_a2 == SMK_edu_independent$other_allele)) #229
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Smoking_Initiation/OUTPUT/WC/"

mvmr_Mario(SMK_edu_independent, wc_outcome, path_)
