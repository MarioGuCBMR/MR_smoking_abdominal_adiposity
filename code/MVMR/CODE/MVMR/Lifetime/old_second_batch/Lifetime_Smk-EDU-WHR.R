...
#Let's check the missing SNPs are the same. 
#If they are we do not need to run the proxies once again:
...
#Perfect match. 
#We can use the WHRAdjBMI proxies :)

####################################
#Great, now let's get those proxies#
####################################
...
whr_proxies_match <- whr[which(whr$chr_pos_37%in%proxies_chr_pos),] #1838
...
lifetime_proxies_match <- lifetime_smk[which(lifetime_smk$chr_pos%in%whr_proxies_match$chr_pos_37),] #1800/1838

#The differences might come from the MHC region, which was removed in curating lifetime data.
...
edu_proxies_match <- edu_proxies_match[which(edu_proxies_match$eaf.outcome > 0.01),] #1791
edu_proxies_match <- edu_proxies_match[which(edu_proxies_match$eaf.outcome < 0.99),] #1790
...
#We end up with 1792! Due to the duplicates, ofc.
...
#In the last version we ended up with 1526, so we have a bit more... 
