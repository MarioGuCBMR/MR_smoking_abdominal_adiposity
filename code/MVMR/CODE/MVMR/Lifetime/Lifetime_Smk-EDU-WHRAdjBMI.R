...
lifetime_edu_df <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/Curated_data/LifeTime_edu_Risk_Factors.txt") #282
...
whradjbmi_match <- whradjbmi[which(whradjbmi$chr_pos_37%in%lifetime_edu_df$chr_pos),] #130
check <- whradjbmi[which(whradjbmi$MarkerName%in%lifetime_edu_df$SNP),] #130 too.

#That means that we need to run proxies for about 150 SNPs.
#Let's go:

missings_snps <- lifetime_edu_df[which(!(lifetime_edu_df$chr_pos%in%whradjbmi$chr_pos_37)),]

#Let's check which of the SNPs are from which dataframe.

missing_lifetime <- missings_snps[which(missings_snps$source == "Lifetime"),] #40/133
...
fwrite(missings_snps, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Lifetime/proxies/missing_snps.txt")
...
setwd("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Lifetime/proxies/")
...
#152! perfect. The query snps are the ones that we think they are.
...
proxies_df_clean_curated <- proxies_df_clean[which(proxies_df_clean$R2 > 0.8),] #7583
...
whradjbmi_proxies_match_lifetime <- whradjbmi[which(whradjbmi$chr_pos_37%in%proxies_df_lifetime$Coord),] #337
whradjbmi_proxies_match_edu <- whradjbmi[which(whradjbmi$chr_pos_37%in%proxies_df_edu$Coord),] #1795
...
lifetime_proxies_match <- lifetime_smk[which(lifetime_smk$chr_pos%in%whradjbmi_proxies_match_lifetime$chr_pos_37),] #336
...
lifetime_edu_df_not_missing <- lifetime_edu_df[which(lifetime_edu_df$chr_pos%in%whradjbmi$chr_pos_37),]

#And here we divide too, by those that are genome-wide significant for each of the SNPs.

lifetime_only_not_missing <- lifetime_edu_df_not_missing[which(lifetime_edu_df_not_missing$source == "Lifetime"),] #32/130
edu_only_not_missing <- lifetime_edu_df_not_missing[which(lifetime_edu_df_not_missing$source == "Education"),] #98/130
...
dat_1_lifetime2edu_proxies <- harmonise_data(lifetime_proxies_match, edu_proxies_match, action = 3) #Note it removes the wrong triallelic combinations.

#Some are palindromic. 
#Let's be wary!!

dat_1_lifetime2edu_proxies <- dat_1_lifetime2edu_proxies[which(dat_1_lifetime2edu_proxies$palindromic == FALSE & dat_1_lifetime2edu_proxies$ambiguous == FALSE | dat_1_lifetime2edu_proxies$palindromic == TRUE & dat_1_lifetime2edu_proxies$ambiguous == FALSE),]
dat_1_lifetime2edu_proxies <- dat_1_lifetime2edu_proxies[which(dat_1_lifetime2edu_proxies$remove == FALSE),] #it removes the incompatible triallelic combination. But keeps the good ones. That is our boy!!

#With this version we get 200 SNPs...

dat_1_lifetime2edu_proxies$samplesize.exposure <- 462690

dat_1_lifetime2edu_proxies_clean <- dat_1_lifetime2edu_proxies %>%
...
colnames(dat_1_lifetime2edu_proxies_clean) <- c("chr_pos", "SNP", "effect_allele", "other_allele", "eaf.smk", "beta.smk", "se.smk", "pval.smk", "samplesize.smk", "eaf.edu", "beta.edu", "se.edu", "pval.edu", "samplesize.edu")

lifetime_edu_independent <- dat_1_lifetime2edu_proxies_clean
...
whradjbmi_outcome <- whradjbmi[which(whradjbmi$MarkerName%in%lifetime_edu_independent$SNP),] #233/233

#1. Let's check that the SNPs are in order:

#Let's check this:

length(which(whradjbmi_outcome$MarkerName != lifetime_edu_independent$SNP)) #0
length(which(whradjbmi_outcome$MarkerName == lifetime_edu_independent$SNP)) #233

length(which(whradjbmi_outcome$chr_pos_37 != lifetime_edu_independent$chr_pos)) #0
length(which(whradjbmi_outcome$MarkerName == lifetime_edu_independent$SNP)) #233
...
whradjbmi_outcome$final_a1 <- ifelse(whradjbmi_outcome$Allele1 != lifetime_edu_independent$effect_allele, whradjbmi_outcome$Allele2, whradjbmi_outcome$Allele1)
whradjbmi_outcome$final_a2 <- ifelse(whradjbmi_outcome$Allele1 != lifetime_edu_independent$effect_allele, whradjbmi_outcome$Allele1, whradjbmi_outcome$Allele2)
whradjbmi_outcome$final_beta <- ifelse(whradjbmi_outcome$Allele1 != lifetime_edu_independent$effect_allele, whradjbmi_outcome$b*(-1), whradjbmi_outcome$b)
whradjbmi_outcome$final_eaf <- ifelse(whradjbmi_outcome$Allele1 != lifetime_edu_independent$effect_allele, 1-whradjbmi_outcome$FreqAllele1HapMapCEU, whradjbmi_outcome$FreqAllele1HapMapCEU)

#Let's check:

length(which(whradjbmi_outcome$final_a1 != lifetime_edu_independent$effect_allele)) #0
length(which(whradjbmi_outcome$final_a1 == lifetime_edu_independent$effect_allele)) #206

length(which(whradjbmi_outcome$final_a2 != lifetime_edu_independent$other_allele)) #0
length(which(whradjbmi_outcome$final_a2 == lifetime_edu_independent$other_allele)) #206
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Lifetime/OUTPUT/WHRAdjBMI"

mvmr_Mario(lifetime_edu_independent, whradjbmi_outcome, path_)
