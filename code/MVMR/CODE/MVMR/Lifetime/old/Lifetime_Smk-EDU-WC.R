...
wc_proxies_match_lifetime <- wc[which(wc$chr_pos_37%in%proxies_df_lifetime$Coord),] #229
wc_proxies_match_edu <- wc[which(wc$chr_pos_37%in%proxies_df_edu$Coord),] #1609

#This is going to be always the final threshold since wcis a bottleneck. Hence...
#we are gonna use these SNPs to match the rest:

lifetime_proxies_match <- lifetime_smk[which(lifetime_smk$chr_pos%in%wc_proxies_match_lifetime$chr_pos_37),] #229
...
#2. Then we checked how many of those are in wcbecause it is the bottleneck.
...
wc_outcome <- wc[which(wc$MarkerName%in%lifetime_edu_independent$SNP),] #199/200

length(which(wc_outcome$MarkerName%in%lifetime_edu_independent$SNP)) #all of them!
...
length(which(wc_outcome$MarkerName == lifetime_edu_independent$SNP)) #199

length(which(wc_outcome$chr_pos_37 != lifetime_edu_independent$chr_pos)) #0
length(which(wc_outcome$MarkerName == lifetime_edu_independent$SNP)) #199
...
length(which(wc_outcome$final_a1 == lifetime_edu_independent$effect_allele)) #199

length(which(wc_outcome$final_a2 != lifetime_edu_independent$other_allele)) #0
length(which(wc_outcome$final_a2 == lifetime_edu_independent$other_allele)) #199
