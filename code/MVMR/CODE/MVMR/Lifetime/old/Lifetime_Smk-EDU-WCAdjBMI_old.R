...
length(which(lifetime_edu_df$pval.smk < 0.00000005)) #68
length(which(lifetime_edu_df$pval.edu < 0.00000005)) #180
...
WCAdjBMI_match <- WCAdjBMI[which(WCAdjBMI$chr_pos_37%in%lifetime_edu_df$chr_pos),] #108
check <- WCAdjBMI[which(WCAdjBMI$MarkerName%in%lifetime_edu_df$SNP),] #108 too.

#That means that we need to run proxies for about 130 SNPs.
#Let's go:

missings_snps <- lifetime_edu_df[which(!(lifetime_edu_df$chr_pos%in%WCAdjBMI$chr_pos_37)),]

########################################################################
#We are gonna check whether the missing SNPs are the as with WHRAdjBMI!#
########################################################################

check_missing_snps <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/Lifetime/proxies/storage/Lifetime_EDU/WHRAdjBMI/missing_snps.txt")

length(which(missings_snps$SNP%in%check_missing_snps$SNP)) #133/133

#Yes, they are the same!! We can use the exact same proxies :)
...
WCAdjBMI_proxies_match <- WCAdjBMI[which(WCAdjBMI$chr_pos_37%in%proxies_chr_pos),] #1804

#This is going to be always the final threshold since whris a bottleneck. Hence...
#we are gonna use these SNPs to match the rest:

lifetime_proxies_match <- lifetime_smk[which(lifetime_smk$chr_pos%in%WCAdjBMI_proxies_match$chr_pos_37),] #1766/1804
...
#2. Then we checked how many of those are in whrbecause it is the bottleneck.
...
colnames(dat_1_smk2edu_proxies_clean) <- c("chr_pos", "chr", "pos", "SNP", "effect_allele", "other_allele", "eaf.smk", "beta.smk", "se.smk", "pval.smk", "samplesize.smk", "eaf.edu", "beta.edu", "se.edu", "pval.edu", "samplesize.edu")

#Only one thing lacking: the minimum p-value to get the proper lead SNP according to each of the phenotypes:

pval_min <- c()

for(i in seq(1, length(dat_1_smk2edu_proxies_clean$SNP))){
  
  pval_vect <- c(dat_1_smk2edu_proxies_clean$pval.smk[i], dat_1_smk2edu_proxies_clean$pval.edu[i])
...
dat_1_smk2edu_proxies_clean$pval <- pval_min #worked perfectly.

###################################
#And finally we can merge the data#
###################################

lifetime_edu_df_not_missing <- lifetime_edu_df[which(lifetime_edu_df$chr_pos%in%WCAdjBMI$chr_pos_37),]

lifetime_edu_df_post_proxies <- rbind(lifetime_edu_df_not_missing, dat_1_smk2edu_proxies_clean)

#And finally we can get the independent ones:
...
#We end up with 206/241. Not bad!!
...
####################################################################
#Now we need to get the data from whraligned and curated too#
####################################################################
...
WCAdjBMI_outcome <- WCAdjBMI[which(WCAdjBMI$MarkerName%in%lifetime_edu_independent$SNP),] #206/206

#1. we order the SNPs:

WCAdjBMI_outcome <- WCAdjBMI_outcome[order(match(WCAdjBMI_outcome$MarkerName, lifetime_edu_independent$SNP)),]

#Let's check this:

length(which(WCAdjBMI_outcome$MarkerName != lifetime_edu_independent$SNP)) #0
length(which(WCAdjBMI_outcome$MarkerName == lifetime_edu_independent$SNP)) #206

#Good.

#2. let's switch the alleles, the beta and the eaf:

WCAdjBMI_outcome$final_a1 <- ifelse(WCAdjBMI_outcome$Allele1 != lifetime_edu_independent$effect_allele, WCAdjBMI_outcome$Allele2, WCAdjBMI_outcome$Allele1)
WCAdjBMI_outcome$final_a2 <- ifelse(WCAdjBMI_outcome$Allele1 != lifetime_edu_independent$effect_allele, WCAdjBMI_outcome$Allele1, WCAdjBMI_outcome$Allele2)
WCAdjBMI_outcome$final_beta <- ifelse(WCAdjBMI_outcome$Allele1 != lifetime_edu_independent$effect_allele, WCAdjBMI_outcome$b*(-1), WCAdjBMI_outcome$b)
WCAdjBMI_outcome$final_eaf <- ifelse(WCAdjBMI_outcome$Allele1 != lifetime_edu_independent$effect_allele, 1-WCAdjBMI_outcome$FreqAllele1HapMapCEU, WCAdjBMI_outcome$FreqAllele1HapMapCEU)

#Let's check:

length(which(WCAdjBMI_outcome$final_a1 != lifetime_edu_independent$effect_allele)) #0
length(which(WCAdjBMI_outcome$final_a1 == lifetime_edu_independent$effect_allele)) #206

length(which(WCAdjBMI_outcome$final_a2 != lifetime_edu_independent$other_allele)) #0
length(which(WCAdjBMI_outcome$final_a2 == lifetime_edu_independent$other_allele)) #206
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/Lifetime/proxies/storage/Lifetime_edu/WCAdjBMI/"

mvmr_Mario(lifetime_edu_independent, WCAdjBMI_outcome, path_)

#DONE! My functions does everything already :)

#Let's check how was the dealio with WHR:

check_conditional_data <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/Lifetime/proxies/storage/Lifetime_edu/WCAdjBMI/conditional_mF_cochrans_Q")
...
BYG <- WCAdjBMI_outcome$final_beta

seBYG <- WCAdjBMI_outcome$se
