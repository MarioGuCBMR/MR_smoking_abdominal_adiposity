...
WCAdjBMI_match <- WCAdjBMI[which(WCAdjBMI$chr_pos_37%in%lifetime_edu_df$chr_pos),] #118
check <- WCAdjBMI[which(WCAdjBMI$MarkerName%in%lifetime_edu_df$SNP),] #118 too.
...
########################################
#Let's get those that are present in WCAdjBMI#
########################################

WCAdjBMI_proxies_match_lifetime <- WCAdjBMI[which(WCAdjBMI$chr_pos_37%in%proxies_df_lifetime$Coord),] #229
WCAdjBMI_proxies_match_edu <- WCAdjBMI[which(WCAdjBMI$chr_pos_37%in%proxies_df_edu$Coord),] #1609

#This is going to be always the final threshold since WCAdjBMIis a bottleneck. Hence...
#we are gonna use these SNPs to match the rest:

lifetime_proxies_match <- lifetime_smk[which(lifetime_smk$chr_pos%in%WCAdjBMI_proxies_match_lifetime$chr_pos_37),] #229
...
#2. Then we checked how many of those are in WCAdjBMIbecause it is the bottleneck.
...
WCAdjBMI_outcome <- WCAdjBMI[which(WCAdjBMI$MarkerName%in%lifetime_edu_independent$SNP),] #199/200

length(which(WCAdjBMI_outcome$MarkerName%in%lifetime_edu_independent$SNP)) #all of them!
...
length(which(WCAdjBMI_outcome$MarkerName == lifetime_edu_independent$SNP)) #199

length(which(WCAdjBMI_outcome$chr_pos_37 != lifetime_edu_independent$chr_pos)) #0
length(which(WCAdjBMI_outcome$MarkerName == lifetime_edu_independent$SNP)) #199
...
length(which(WCAdjBMI_outcome$final_a1 == lifetime_edu_independent$effect_allele)) #199

length(which(WCAdjBMI_outcome$final_a2 != lifetime_edu_independent$other_allele)) #0
length(which(WCAdjBMI_outcome$final_a2 == lifetime_edu_independent$other_allele)) #199
