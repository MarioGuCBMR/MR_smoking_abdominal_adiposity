...
whradjbmi_match <- whradjbmi[which(whradjbmi$chr_pos_37%in%lifetime_edu_df$chr_pos),] #118
check <- whradjbmi[which(whradjbmi$MarkerName%in%lifetime_edu_df$SNP),] #118 too.
...
missing_lifetime <- missings_snps[which(missings_snps$source == "Lifetime"),] #39/133
missing_edu <- missings_snps[which(missings_snps$source == "Education"),] #94/133

#This sums 133. Now is perfect.
...
fwrite(missings_snps, "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/Lifetime/proxies/storage/Lifetime_EDU/WHRAdjBMI/missing_snps.txt")
...
setwd("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/Lifetime/proxies/storage/Lifetime_EDU/WHRAdjBMI/")
...
proxies_df_clean_curated <- proxies_df_clean[which(proxies_df_clean$R2 > 0.8),] #6603

#Now we are gonna divide the SNPs depending on the query, whether it was from lifetime or it was from education.

proxies_df_lifetime <- proxies_df_clean_curated[which(proxies_df_clean_curated$query_snp%in%missing_lifetime$chr_pos),] #649
proxies_df_edu <- proxies_df_clean_curated[which(proxies_df_clean_curated$query_snp%in%missing_edu$chr_pos),] #5954

###############################################
#Let's get those that are present in WHRAdjBMI#
###############################################

whradjbmi_proxies_match_lifetime <- whradjbmi[which(whradjbmi$chr_pos_37%in%proxies_df_lifetime$Coord),] #229
whradjbmi_proxies_match_edu <- whradjbmi[which(whradjbmi$chr_pos_37%in%proxies_df_edu$Coord),] #1609
...
lifetime_proxies_match <- lifetime_smk[which(lifetime_smk$chr_pos%in%whradjbmi_proxies_match_lifetime$chr_pos_37),] #229
...
#We get 214, but we have to be wary since there are duplicates and maybe palindromic ones.
...
whradjbmi_outcome <- whradjbmi[which(whradjbmi$MarkerName%in%lifetime_edu_independent$SNP),] #199/200

length(which(whradjbmi_outcome$MarkerName%in%lifetime_edu_independent$SNP)) #all of them!

#Let's take out the duplicate according to education pvalue:

lifetime_edu_independent <- lifetime_edu_independent[order(lifetime_edu_independent$pval.edu),]

lifetime_edu_independent <- lifetime_edu_independent[-which(duplicated(lifetime_edu_independent$SNP) == TRUE),]

#1. we order the SNPs:

whradjbmi_outcome <- whradjbmi_outcome[order(match(whradjbmi_outcome$MarkerName, lifetime_edu_independent$SNP)),]

#Let's check this:

length(which(whradjbmi_outcome$MarkerName != lifetime_edu_independent$SNP)) #0
length(which(whradjbmi_outcome$MarkerName == lifetime_edu_independent$SNP)) #206
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/Lifetime/proxies/storage/Lifetime_edu/WHRAdjBMI/"
