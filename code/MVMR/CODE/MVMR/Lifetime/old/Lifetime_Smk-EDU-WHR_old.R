...
lifetime_edu_df <- fread("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/Curated_data/LifeTime_edu_Risk_Factors.txt") #251
...
whr_match <- whr[which(whr$chr_pos_37%in%lifetime_edu_df$chr_pos),] #118
check <- whr[which(whr$MarkerName%in%lifetime_edu_df$SNP),] #118 too.
...
whr_proxies_match <- whr[which(whr$chr_pos_37%in%proxies_chr_pos),] #1804
...
lifetime_proxies_match <- lifetime_smk[which(lifetime_smk$chr_pos%in%whr_proxies_match$chr_pos_37),] #1766/1804
...
whr_outcome <- whr[which(whr$MarkerName%in%lifetime_edu_independent$SNP),] #206/206
...
length(which(whr_outcome$MarkerName == lifetime_edu_independent$SNP)) #206
...
length(which(whr_outcome$final_a1 == lifetime_edu_independent$effect_allele)) #206

length(which(whr_outcome$final_a2 != lifetime_edu_independent$other_allele)) #0
length(which(whr_outcome$final_a2 == lifetime_edu_independent$other_allele)) #206
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/Lifetime/proxies/storage/Lifetime_edu/WHR/"
...
check_conditional_data <- readRDS("J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/Lifetime/proxies/storage/Lifetime_edu/WHR/conditional_mF_cochrans_Q")
...
Pcov_0 <- matrix(data = c(1, 0, 0 , 1), nrow = 2, ncol = 2) #we make the fake phenotypic correlations
Pcov_0.05 <- matrix(data = c(1, 0.25, 0.25 , 1), nrow = 2, ncol = 2) #we make the fake phenotypic correlations
Pcov_0.10 <- matrix(data = c(1, 0.5, 0.5 , 1), nrow = 2, ncol = 2) #we make the fake phenotypic correlations
Pcov_0.15 <- matrix(data = c(1, 0.75, 0.75 , 1), nrow = 2, ncol = 2) #we make the fake phenotypic correlations
Pcov_0.20 <- matrix(data = c(1, 1, 1 , 1), nrow = 2, ncol = 2) #we make the fake phenotypic correlations
...
BYG <- whr_outcome$final_beta

seBYG <- whr_outcome$se
...
pleoitropy_0.25 <- pleiotropy_mvmr(final_df, covariance_matrix_0.05) #same

#Q-Statistic for instrument validity:
#  298.7749 on 204 DF , p-value: 1.683962e-05

pleoitropy_0.50 <- pleiotropy_mvmr(final_df, covariance_matrix_0.10) #same

#Q-Statistic for instrument validity:
#  299.6242 on 204 DF , p-value: 1.459209e-05

pleoitropy_0.75 <- pleiotropy_mvmr(final_df, covariance_matrix_0.15) #same

#Q-Statistic for instrument validity:
#  300.4792 on 204 DF , p-value: 1.262265e-05

pleoitropy_1 <- pleiotropy_mvmr(final_df, covariance_matrix_0.20) #same.
...
qhet_mvmr(final_df, pcor = Pcov_0.05)

qhet_mvmr(final_df, pcor = Pcov_0.100)

qhet_mvmr(final_df, pcor = Pcov_0.15)

qhet_mvmr(final_df, pcor = Pcov_0.20)
...
outliers_0.05 <- mvmr_cochran_Q(final_df, covariance_matrix_0.05)
outliers_0.5 <- mvmr_cochran_Q(final_df, covariance_matrix_0.10) 
outliers_0.15 <- mvmr_cochran_Q(final_df, covariance_matrix_0.15) 
outliers_0.20 <- mvmr_cochran_Q(final_df, covariance_matrix_0.20) 
