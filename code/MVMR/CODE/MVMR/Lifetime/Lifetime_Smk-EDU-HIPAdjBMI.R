...
hipadjbmi_match <- hipadjbmi[ which(hipadjbmi$chr_pos%in%lifetime_edu_df$chr_pos),] #130
check <- hipadjbmi[ which(hipadjbmi$MarkerName%in%lifetime_edu_df$SNP),] #130 too.

#That means that we need to run proxies for about 130 SNPs.
#Let's go:

missings_snps <- lifetime_edu_df[which(!(lifetime_edu_df$chr_pos%in%hipadjbmi$ chr_pos_37)),]
...
hipadjbmi_proxies_match_lifetime <- hipadjbmi[which(hipadjbmi$chr_pos_37%in%proxies_df_lifetime$Coord),] #377
hipadjbmi_proxies_match_edu <- hipadjbmi[which(hipadjbmi$chr_pos_37%in%proxies_df_edu$Coord),] #1795
...
lifetime_proxies_match <- lifetime_smk[which(lifetime_smk$chr_pos%in%hipadjbmi_proxies_match_lifetime$chr_pos_37),] #336
...
lifetime_edu_df_not_missing <- lifetime_edu_df[which(lifetime_edu_df$chr_pos%in%hipadjbmi$ chr_pos_37),]
...
hipadjbmi_outcome <- hipadjbmi[which(hipadjbmi$MarkerName%in%lifetime_edu_independent$SNP),] #233/233

length(which(hipadjbmi_outcome$MarkerName%in%lifetime_edu_independent$SNP)) #all of them!

#1. we order the SNPs:

hipadjbmi_outcome <- hipadjbmi_outcome[order(match(hipadjbmi_outcome$MarkerName, lifetime_edu_independent$SNP)),]

#Let's check this:

length(which(hipadjbmi_outcome$MarkerName != lifetime_edu_independent$SNP)) #0
length(which(hipadjbmi_outcome$MarkerName == lifetime_edu_independent$SNP)) #233

length(which(hipadjbmi_outcome$chr_pos_37 != lifetime_edu_independent$chr_pos)) #0
length(which(hipadjbmi_outcome$MarkerName == lifetime_edu_independent$SNP)) #233
...
hipadjbmi_outcome$final_a1 <- ifelse(hipadjbmi_outcome$Allele1 != lifetime_edu_independent$effect_allele, hipadjbmi_outcome$Allele2, hipadjbmi_outcome$Allele1)
hipadjbmi_outcome$final_a2 <- ifelse(hipadjbmi_outcome$Allele1 != lifetime_edu_independent$effect_allele, hipadjbmi_outcome$Allele1, hipadjbmi_outcome$Allele2)
hipadjbmi_outcome$final_beta <- ifelse(hipadjbmi_outcome$Allele1 != lifetime_edu_independent$effect_allele, hipadjbmi_outcome$b*(-1), hipadjbmi_outcome$b)
hipadjbmi_outcome$final_eaf <- ifelse(hipadjbmi_outcome$Allele1 != lifetime_edu_independent$effect_allele, 1-hipadjbmi_outcome$FreqAllele1HapMapCEU, hipadjbmi_outcome$FreqAllele1HapMapCEU)

#Let's check:

length(which(hipadjbmi_outcome$final_a1 != lifetime_edu_independent$effect_allele)) #0
length(which(hipadjbmi_outcome$final_a1 == lifetime_edu_independent$effect_allele)) #233

length(which(hipadjbmi_outcome$final_a2 != lifetime_edu_independent$other_allele)) #0
length(which(hipadjbmi_outcome$final_a2 == lifetime_edu_independent$other_allele)) #233
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Lifetime/OUTPUT/HIPAdjBMI/"

mvmr_Mario(lifetime_edu_independent, hipadjbmi_outcome, path_)
