...
hip_match <- hip[ which(hip$chr_pos_37%in%lifetime_edu_df$chr_pos),] #130
check <- hip[ which(hip$MarkerName%in%lifetime_edu_df$SNP),] #130 too.

#That means that we need to run proxies for about 130 SNPs.
#Let's go:

missings_snps <- lifetime_edu_df[which(!(lifetime_edu_df$chr_pos%in%hip$ chr_pos_37)),]
...
hip_proxies_match_lifetime <- hip[which(hip$chr_pos_37%in%proxies_df_lifetime$Coord),] #377
hip_proxies_match_edu <- hip[which(hip$chr_pos_37%in%proxies_df_edu$Coord),] #1795
...
lifetime_proxies_match <- lifetime_smk[which(lifetime_smk$chr_pos%in%hip_proxies_match_lifetime$chr_pos_37),] #336
...
lifetime_edu_df_not_missing <- lifetime_edu_df[which(lifetime_edu_df$chr_pos%in%hip$ chr_pos_37),]
...
hip_outcome <- hip[which(hip$MarkerName%in%lifetime_edu_independent$SNP),] #233/233

length(which(hip_outcome$MarkerName%in%lifetime_edu_independent$SNP)) #all of them!

#1. we order the SNPs:

hip_outcome <- hip_outcome[order(match(hip_outcome$MarkerName, lifetime_edu_independent$SNP)),]

#Let's check this:

length(which(hip_outcome$MarkerName != lifetime_edu_independent$SNP)) #0
length(which(hip_outcome$MarkerName == lifetime_edu_independent$SNP)) #233

length(which(hip_outcome$chr_pos_37 != lifetime_edu_independent$chr_pos)) #0
length(which(hip_outcome$MarkerName == lifetime_edu_independent$SNP)) #233
...
hip_outcome$final_a1 <- ifelse(hip_outcome$Allele1 != lifetime_edu_independent$effect_allele, hip_outcome$Allele2, hip_outcome$Allele1)
hip_outcome$final_a2 <- ifelse(hip_outcome$Allele1 != lifetime_edu_independent$effect_allele, hip_outcome$Allele1, hip_outcome$Allele2)
hip_outcome$final_beta <- ifelse(hip_outcome$Allele1 != lifetime_edu_independent$effect_allele, hip_outcome$b*(-1), hip_outcome$b)
hip_outcome$final_eaf <- ifelse(hip_outcome$Allele1 != lifetime_edu_independent$effect_allele, 1-hip_outcome$FreqAllele1HapMapCEU, hip_outcome$FreqAllele1HapMapCEU)

#Let's check:

length(which(hip_outcome$final_a1 != lifetime_edu_independent$effect_allele)) #1
length(which(hip_outcome$final_a1 == lifetime_edu_independent$effect_allele)) #232

length(which(hip_outcome$final_a2 != lifetime_edu_independent$other_allele)) #1
length(which(hip_outcome$final_a2 == lifetime_edu_independent$other_allele)) #232

#One of them is not matching!!

which(hip_outcome$final_a1 != lifetime_edu_independent$effect_allele) #89

hip_outcome[89,]
lifetime_edu_independent[89,]

#They are the same SNP, but..., they are on different strands!!

#Really easy, now we can do the following:

lifetime_edu_independent$effect_allele[89] <- "G"
lifetime_edu_independent$other_allele[89] <- "T"
...
length(which(hip_outcome$final_a1 != lifetime_edu_independent$effect_allele)) #0
length(which(hip_outcome$final_a1 == lifetime_edu_independent$effect_allele)) #232

length(which(hip_outcome$final_a2 != lifetime_edu_independent$other_allele)) #0
length(which(hip_outcome$final_a2 == lifetime_edu_independent$other_allele)) #232

head(hip_outcome) #worked like a charm. 
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Lifetime/OUTPUT/HIP/"

mvmr_Mario(lifetime_edu_independent, hip_outcome, path_)
