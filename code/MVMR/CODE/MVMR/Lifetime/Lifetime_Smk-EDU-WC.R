...
wc_match <- wc[ which(wc$chr_pos_37%in%lifetime_edu_df$chr_pos),] #130
check <- wc[ which(wc$MarkerName%in%lifetime_edu_df$SNP),] #130 too.
...
wc_proxies_match_lifetime <- wc[which(wc$chr_pos_37%in%proxies_df_lifetime$Coord),] #377
wc_proxies_match_edu <- wc[which(wc$chr_pos_37%in%proxies_df_edu$Coord),] #1795
...
lifetime_proxies_match <- lifetime_smk[which(lifetime_smk$chr_pos%in%wc_proxies_match_lifetime$chr_pos_37),] #336
...
wc_outcome <- wc[which(wc$MarkerName%in%lifetime_edu_independent$SNP),] #233/233
...
length(which(wc_outcome$MarkerName == lifetime_edu_independent$SNP)) #233
...
length(which(wc_outcome$final_a1 == lifetime_edu_independent$effect_allele)) #233

length(which(wc_outcome$final_a2 != lifetime_edu_independent$other_allele)) #0
length(which(wc_outcome$final_a2 == lifetime_edu_independent$other_allele)) #233
...
path_ <- "J:/CBMR/SUN-CBMR-Kilpelainen-Group/Team projects/Mario&German/Smoking_WHR/CODE_4_Replication/MVMR/CODE/MVMR/Lifetime/OUTPUT/WC/"
